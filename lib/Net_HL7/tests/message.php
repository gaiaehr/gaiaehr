...
ok($seg0->getName() == "MSH", "Segment 0 name MSH<br>");
ok($seg1->getName() == "PID", "Segment 1 name PID<br>");
ok($seg0->getField(3) == "XXX", "3d field of MSH<br>");
ok($seg1->getField(2) == "Foo", "2nd field of PID<br>");

// Check references
$segX = $msg->getSegmentByIndex(0);
ok($segX->getField(3) == "XXX", "3d field of MSH<br>");

$msg = new Net_HL7_Message("MSH|^~\\&|1|\rPID|||xxx|\r");

$seg0 = $msg->getSegmentByIndex(0);

ok($msg->toString() == "MSH|^~\\&|1|\rPID|||xxx|\r", "String representation of message<br>");

ok($msg->toString(1) == "MSH|^~\\&|1|\nPID|||xxx|\n", "Pretty print representation of message<br>");

ok($seg0->getField(2) == "^~\\&", "Encoding characters (MSH(2))<br>");
...
ok($comps[0] == "xx", "Composed field<br>");
ok($comps[1][1] == "y", "Subcomposed field<br>");

# Trying different field separator
#
$msg = new Net_HL7_Message("MSH*^~\\&*1\rPID***xxx\r");

ok($msg->toString() == "MSH*^~\\&*1*\rPID***xxx*\r", "String representation of message with * as field separator<br>");

$seg0 = $msg->getSegmentByIndex(0);

ok($seg0->getField(3) == "1", "3d field of MSH<br>");
...
ok($comps[0] == "x", "Composed field with . as separator<br>");
ok($comps[1][1] == "y", "Subcomposed field with @ as separator<br>");
...
ok($seg1->getName() == $seg2->getName(), "Add/remove segment<br>");
...
ok($seg3->getName() == $seg2->getName(), "Insert segment<br>");
...
ok(! $msg->getSegmentByIndex(3), "Erroneous insert<br>");

$msg->insertSegment($seg5, 2);

$seg2 = $msg->getSegmentByIndex(2);

ok($seg2->getName() == $seg5->getName(), "Insert segment<br>");

$msg->setSegment($seg3, 2);

$seg2 = $msg->getSegmentByIndex(2);

ok($seg2->getName() == $seg3->getName(), "Set segment<br>");

ok(count($msg->getSegmentsByName("MSH")) == 1, "Number of MSH segments<br>");
