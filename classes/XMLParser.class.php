 ...
 *
 * Parses an XML document into an object structure much like the SimpleXML
 * extension.
 *
 ...
 * array. They are also stored by
 * type as arrays. So, if, for example, this tag had 2 <font> tags as children,
 * there would be a class member
 * called $font created as an array. $font[0] would be the first font tag, and
 * $font[1] would be the second.
 *
 * To loop through all of the direct children of this object, the $children
 * member should be used.
 *
 * To loop through all of the direct children of a specific tag for this object,
...
class XMLTag
{
	 ...
	 *
	 * So, if the tag doesn't contain child tags, and just contains a string, it
	 * would go here
	 *
	 * @var stat
	 */
	...
	/**
	 * The number of parents this XML object has (number of levels from this tag to
	 * the root tag)
	 *
	...
	/**
	 * Returns the string of the XML document which would be generated from this
	 * object
	 *
	 * This function works recursively, so it gets the XML of itself and all of its
	 * children, which
	 * in turn gets the XML of all their children, which in turn gets the XML of all
	 * thier children,
	 * and so on. So, if you call GetXML from the document root object, it will
	 * return a string for
	 * the XML of the entire document.
	 *
	 * This function does not, however, return a DTD or an XML version/encoding tag.
	 * That should be
	...
	{
		//Start a new line, indent by the number indicated in $this->parents, add a <,
		// and add the name of the tag
		...
		else
		{
			...
			if (!empty($this -> tagChildren))
			{
				...
				//For each child, call the GetXML function (this will ensure that all children
