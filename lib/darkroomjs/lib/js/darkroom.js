...
(function(window, document, fabric){
	...
	Darkroom.prototype = {
		...
		init: function(element, options, plugins){
			...
			plugins = plugins || Darkroom.plugins;
		...
		initDOM: function(element){
			...
			this.scroller = new Scroller(function(left, top, zoom){
				...
				me.image.setWidth(width / zoom);
				me.image.setHeight(height / zoom);
				me.canvas.moveObject(me.image, _left, _top);
				me.image.setCoords();
				me.canvas.renderAll();
				me.dispatchEvent('image:change');
			...
			canvasContainer.addEventListener("mousedown", function(e) {
				if (e.target.tagName.match(/input|textarea|select/i) || me.isKeyCroping) {
					return;
				}
			...
			canvasContainer.addEventListener(navigator.userAgent.indexOf("Firefox") > -1 ? "DOMMouseScroll" :  "mousewheel", function(e) {
				me.scroller.doMouseZoom(e.detail ? (e.detail * -120) : e.wheelDelta, e.timeStamp, e.pageX, e.pageY);
			}, false);
		...
		selfDestroy: function(){
			...
			if(this.originalImage){
				this.originalImage.src = this.snapshotImage();
				this.originalImage.style.display = null;
