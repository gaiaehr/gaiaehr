...
class dbHelper
{
	...
	function __construct()
	{
		...
		if(isset($_SESSION['site']['db'])){
			...
			try{
				$this->conn = new PDO('mysql:host=' . $host . ';port=' . $port . ';dbname=' . $dbName, $dbUser, $dbPass, array(PDO::MYSQL_ATTR_LOCAL_INFILE => 1, PDO::ATTR_PERSISTENT => true));
	...
	public function sqlBind($BindFieldsArray, $Table, $InsertOrUpdate = 'I', $Where = null)
	{
		...
		if(isset($BindFieldsArray['GaiaEHR'])) unset($BindFieldsArray['GaiaEHR']);
		/**
		...
		foreach($BindFieldsArray as $key => $value){
			...
			if(isset($Where) && is_array($Where)){
				if(!array_key_exists($key, $Where)){
					...
					return array(
						'success' => false, 'error' => 'Where value can not be updated. please make sure to unset it from the array'
					);
				}
			} else {
