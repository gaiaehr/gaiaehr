...
Ext.define('Extensible',
{
	...
	Date :
	{
		...
		{
			var denom = 1, diff = end.getTime() - start.getTime();
		...
		{
			var day = 1000 * 60 * 60 * 24, clear = Ext.Date.clearTime, diff = clear(end, true).getTime() - clear(start, true).getTime();
		...
		copyTime : function(fromDt, toDt)
		{
			...
			dt.setHours(fromDt.getHours(), fromDt.getMinutes(), fromDt.getSeconds(), fromDt.getMilliseconds());
		...
		add : function(dt, o)
		{
			...
			var ExtDate = Ext.Date, dateAdd = ExtDate.add, newDt = ExtDate.clone(dt);
	...
	if (extVersion.isLessThan('4.1'))
	{
		if (Ext.data && Ext.data.reader && Ext.data.reader.Reader)
		{
			Ext.data.reader.Reader.override(
			{
				...
				{
					var me = this, values = [], records = [], Model = me.model, i = 0, length = root.length, idProp = me.getIdProperty(), node, id, record;
...
Ext.define('Extensible.data.Model',
{
	...
	inheritableStatics :
	{
		...
		{
			var proto = this.prototype, mappings = Ext.ClassManager.get(proto.mappingClass || ''), idProperty = proto.mappingIdProperty, prop, fields = [], i = 0, len = 0;
	...
	{
		var copy = Ext.create(this.$className), dataProp = this.persistenceProperty;

		copy[dataProp] = Ext.Object.merge(
		{
		}, this[dataProp]);
...
Ext.define('Extensible.form.recurrence.option.Duration',
{
	...
	requires : ['Ext.form.Label', 'Ext.form.field.ComboBox', 'Ext.form.field.Number', 'Ext.form.field.Date'],
	...
	{
		var me = this, startDate = me.getStartDate();
	...
	{
		return Extensible.Date.add(untilDate,
		{
			days : 1,
			seconds : -1
		});
	...
	setValue : function(v)
	{
		...
		var options = Ext.isArray(v) ? v : v.split(me.optionDelimiter), didSetValue = false, parts;
...
Ext.define('Extensible.form.recurrence.option.Interval',
{
	...
	setValue : function(v)
	{
		...
		var options = Ext.isArray(v) ? v : v.split(me.optionDelimiter), parts;
	...
	updateLabel : function(unit)
	{
		...
		if (me.intervalField)
		{
			...
			if (me.dateLabel)
			{
				me.dateLabel.update(me.unit + s + ' beginning ' + Ext.Date.format(me.getStartDate(), me.dateLabelFormat));
			}
		}
		return me;
	}
...
Ext.define('Extensible.form.recurrence.option.Monthly',
{
	...
	requires : ['Ext.form.field.ComboBox', 'Extensible.lang.Number'],
	...
	initNthCombo : function()
	{
		...
		var me = this, combo = me.nthCombo, store = combo.store, dt = me.getStartDate(),
		...
		data = [[monthDayText + yearlyText, me.isYearly ? byMonthValue : 'BYMONTHDAY=' + dayNum], [dayOfWeekText + yearlyText, byMonthValue + delimiter + 'BYDAY=' + dayIndex + dayNameAbbreviated]],
		...
		if (lastDayOfMonth - dayNum < 7)
		{
			...
			data.push(['last ' + Ext.Date.format(dt, 'l') + yearlyText, byMonthValue + delimiter + 'BYDAY=-1' + dayNameAbbreviated]);
		}
	...
	setValue : function(v)
	{
		...
		var options = Ext.isArray(v) ? v : v.split(me.optionDelimiter), parts, values = [];
...
Ext.define('Extensible.form.recurrence.option.Weekly',
{
	...
	requires : ['Ext.form.field.Checkbox', // should be required by CheckboxGroup but isn't
	...
	getValue : function()
	{
		...
		if (me.daysCheckboxGroup)
		{
			...
			var fieldValue = me.daysCheckboxGroup.getValue(), days = [], property;
	...
	setValue : function(v)
	{
		...
		var options = Ext.isArray(v) ? v : v.split(me.optionDelimiter), compositeValue =
		{
		}, parts, days;
...
Ext.define('Extensible.form.recurrence.FrequencyCombo',
{
	...
	initComponent : function()
	{
		 ...
		 *    ]
		 */
		me.frequencyOptions = me.frequencyOptions || [['NONE', me.frequencyText.none], ['DAILY', me.frequencyText.daily], ['WEEKDAYS', me.frequencyText.weekdays], ['WEEKLY', me.frequencyText.weekly], ['MONTHLY', me.frequencyText.monthly], ['YEARLY', me.frequencyText.yearly]];
...
Ext.define('Extensible.form.recurrence.Fieldset',
{
	...
	requires : ['Ext.form.Label', 'Extensible.form.recurrence.FrequencyCombo', 'Extensible.form.recurrence.option.Interval', 'Extensible.form.recurrence.option.Weekly', 'Extensible.form.recurrence.option.Monthly', 'Extensible.form.recurrence.option.Yearly', 'Extensible.form.recurrence.option.Duration'],
	...
	options : ['daily', 'weekly', 'weekdays', 'monthly', 'yearly'],
...
Ext.define('Extensible.form.recurrence.RangeEditPanel',
{
	...
	initComponent : function()
	{
		...
		me.items = [me.getHeaderConfig(), me.getOptionPanelConfig(), me.getSummaryConfig()];
		me.callParent(arguments);
	},
	...
	showEditModes : function(modes)
	{
		...
		var me = this, i = 0, btn, len = modes.length;
	...
	{
		var me = this, summaryEl = me.getComponent(me.id + '-summary').getEl();
...
Ext.define('Extensible.form.recurrence.RangeEditWindow',
{
	...
	requires : ['Extensible.form.recurrence.RangeEditPanel'],
...
Ext.define('Extensible.calendar.template.BoxLayout',
{
	...
	applyTemplate : function(o)
	{
		...
		var w = 0, title = '', first = true, isToday = false, showMonth = false, prevMonth = false, nextMonth = false, isWeekend = false, weekendCls = o.weekendCls, prevMonthCls = o.prevMonthCls, nextMonthCls = o.nextMonthCls, todayCls = o.todayCls, weeks = [[]], today = Extensible.Date.today(), dt = Ext.Date.clone(this.viewStart), thisMonth = this.startDate.getMonth();
		...
		for (; w < this.weekCount || this.weekCount == -1; w++)
		{
			...
			for (var d = 0; d < this.dayCount; d++)
			{
				...
				if (showMonth)
				{
					...
					{
						title = Ext.Date.format(dt, this.dayCount == 1 ? this.singleDayDateFormat : ( first ? this.multiDayFirstDayFormat : this.multiDayMonthStartFormat));
					}
				}
				else
				{
				...
				weeks[w].push(
				{
					...
					titleCls : 'ext-cal-dtitle ' + ( isToday ? ' ext-cal-dtitle-today' : '') + (w == 0 ? ' ext-cal-dtitle-first' : '') + ( prevMonth ? ' ext-cal-dtitle-prev' : '') + ( nextMonth ? ' ext-cal-dtitle-next' : ''),
					cellCls : 'ext-cal-day ' + ( isToday ? ' ' + todayCls : '') + (d == 0 ? ' ext-cal-day-first' : '') + ( prevMonth ? ' ' + prevMonthCls : '') + ( nextMonth ? ' ' + nextMonthCls : '') + (isWeekend && weekendCls ? ' ' + weekendCls : '')
				});
	...
	{
		var timeFmt = Extensible.Date.use24HourTime ? 'G:i ' : 'g:ia ', todayText = this.showTodayText !== false ? this.todayText : '', timeText = this.showTime !== false ? ' <span id="' + this.id + '-clock" class="ext-cal-dtitle-time" aria-live="off">' + Ext.Date.format(new Date(), timeFmt) + '</span>' : '', separator = todayText.length > 0 || timeText.length > 0 ? ' &#8212; ' : '';
		// &#8212; == &mdash;
...
Ext.define('Extensible.calendar.template.DayBody',
{
	...
	applyTemplate : function(o)
	{
		...
		var i = 0, days = [], dt = Ext.Date.clone(o.viewStart);

		for (; i < this.dayCount; i++)
		{
			days[i] = Extensible.Date.add(dt,
			{
				days : i
			});
		}

		var times = [], start = this.viewStartHour, end = this.viewEndHour, mins = this.hourIncrement, dayHeight = this.hourHeight * (end - start), fmt = Extensible.Date.use24HourTime ? 'G:i' : 'ga', templateConfig;

		// use a fixed DST-safe date so times don't get skipped on DST boundaries
		dt = Extensible.Date.add(new Date('5/26/1972'),
		{
			hours : start
		});
...
Ext.define('Extensible.calendar.template.Month',
{
	...
	applyTemplate : function(o)
	{
		var days = [], weeks = this.weekTpl.apply(o), dt = o.viewStart, D = Extensible.Date, templateConfig;
...
Ext.define('Ext.dd.ScrollManager',
{
	...
	onFire : function(e, isDrop)
	{
		...
		var xy = e.getXY(), pt = e.getPoint(), proc = this.proc, els = this.els;
...
Ext.define('Extensible.calendar.dd.StatusProxy',
{
	...
	renderTpl : ['<div class="' + Ext.baseCSSPrefix + 'dd-drop-icon"></div>', '<div class="ext-dd-ghost-ct">', '<div id="{id}-ghost" class="' + Ext.baseCSSPrefix + 'dd-drag-ghost"></div>', '<div id="{id}-message" class="ext-dd-msg"></div>', '</div>'],
...
Ext.define('Extensible.calendar.dd.DragZone',
{
	...
	requires : ['Extensible.calendar.dd.StatusProxy', 'Extensible.calendar.data.EventMappings'],
	...
	onInitDrag : function(x, y)
	{
		...
		{
			var ghost = this.dragData.ddel.cloneNode(true), child = Ext.fly(ghost).down('dl');
...
Ext.define('Extensible.calendar.dd.DropZone',
{
	...
	requires : ['Ext.Layer', 'Extensible.calendar.data.EventMappings'],
	...
	{
		var dragOffset = this.dragOffset || 0, y = e.getPageY() - dragOffset, d = this.view.getDayAt(e.getPageX(), y);
	...
	{
		var D = Extensible.Date, eventDragText = (e.ctrlKey || e.altKey) ? this.copyText : this.moveText, start = data.type == 'eventdrag' ? n.date : D.min(data.start, n.date), end = data.type == 'eventdrag' ? D.add(n.date,
		{
		...
		data.proxy.updateMsg(Ext.String.format(data.type === 'eventdrag' ? eventDragText : this.createText, this.currentRange));
	...
	{
		this.currWeek = -1;
		var dt = Ext.Date.clone(start), i = 0, shim, box, D = Extensible.Date, cnt = D.diffDays(dt, end) + 1;
		...
		while (i++ < cnt)
		{
			...
			if (dayEl)
			{
				var wk = this.view.getWeekIndex(dt), shim = this.shims[wk];
	...
	onNodeDrop : function(n, dd, e, data)
	{
		if (n && data)
		{
			...
			{
				var rec = this.view.getEventRecordFromEl(data.ddel), dt = Extensible.Date.copyTime(rec.data[Extensible.calendar.data.EventMappings.StartDate.name], n.date);
			...
			if (data.type == 'caldrag')
			{
				if (!this.dragEndDate)
				{
					...
					this.dragEndDate = Extensible.Date.add(this.dragStartDate,
					{
						days : 1,
						millis : -1,
						clearTime : true
...
Ext.define('Extensible.calendar.dd.DayDragZone',
{
	...
	getDragData : function(e)
	{
		...
		if (t)
		{
			var p = t.parent(this.eventSelector), rec = this.view.getEventRecordFromEl(p);
...
Ext.define('Extensible.calendar.dd.DayDropZone',
{
	...
	onNodeOver : function(n, dd, e, data)
	{
		var dt, text = this.createText, timeFormat = Extensible.Date.use24HourTime ? 'G:i' : 'g:ia';
		...
		if (data.type == 'caldrag')
		{
			...
			if (e.getY() < box.y)
			{
				...
				endDt = Extensible.Date.add(this.dragCreateDt,
				{
					minutes : this.ddIncrement
				});
			}
			else
			{
				n.date = Extensible.Date.add(n.date,
				{
					minutes : this.ddIncrement
				});
			}
			...
			var diff = Extensible.Date.diff(this.dragCreateDt, n.date), curr = Extensible.Date.add(this.dragCreateDt,
			{
				millis : diff
			});
			...
			dt = Ext.String.format(this.dateRangeFormat, Ext.Date.format(this.dragStartDate, timeFormat), Ext.Date.format(this.dragEndDate, timeFormat));
		}
		else
		{
			var evtEl = Ext.get(data.ddel), dayCol = evtEl.parent().parent(), box = evtEl.getBox();
			...
			if (data.type == 'eventresize')
			{
				...
				var diff = Extensible.Date.diff(this.resizeDt, n.date), curr = Extensible.Date.add(this.resizeDt,
				{
					millis : diff
				}), start = Extensible.Date.min(data.eventStart, curr), end = Extensible.Date.max(data.eventStart, curr);
				...
				dt = Ext.String.format(this.dateRangeFormat, Ext.Date.format(start, timeFormat), Ext.Date.format(end, timeFormat));
...
Ext.define('Extensible.calendar.data.EventModel',
{
	...
	getEndDate : function()
	{
		var EventMappings = Extensible.calendar.data.EventMappings, duration = EventMappings.Duration ? this.get(EventMappings.Duration.name) : null;
...
Ext.define('Extensible.calendar.data.MemoryCalendarStore',
{
	...
	requires : ['Ext.data.proxy.Memory', 'Ext.data.reader.Json', 'Ext.data.writer.Json', 'Extensible.calendar.data.CalendarModel', 'Extensible.calendar.data.CalendarMappings'],
...
Ext.define('Extensible.calendar.data.MemoryEventStore',
{
	...
	requires : ['Ext.data.proxy.Memory', 'Ext.data.reader.Json', 'Ext.data.writer.Json', 'Extensible.calendar.data.EventModel', 'Extensible.calendar.data.EventMappings'],
	...
	onWrite : function(store, operation)
	{
		...
		{
			var success = operation.wasSuccessful(), rec = operation.records[0], title = rec.data[Extensible.calendar.data.EventMappings.Title.name];
	...
	{
		var me = this, successful = operation.wasSuccessful(), resultSet = operation.getResultSet(), records = [];
...
Ext.define('Extensible.calendar.util.WeekEventRenderer',
{
	...
	statics :
	{
		...
		getEventRow : function(viewId, weekIndex, rowIndex)
		{
			...
			weekRow = Ext.get(viewId + '-wk-' + weekIndex), eventRow, weekTable;
		...
		{
			var eventMappings = Extensible.calendar.data.EventMappings, eventData = event.data || event.event.data, startOfWeek = Ext.Date.clone(currentDate), endOfWeek = Extensible.Date.add(startOfWeek,
			{
				days : dayCount - dayIndex,
				millis : -1
			}), eventRow = this.getEventRow(renderConfig.viewId, weekIndex, eventIndex), eventEndDate = (event.event || event).getEndDate(), daysToEventEnd = Extensible.Date.diffDays(currentDate, eventEndDate) + 1,
			...
			eventData.spanCls = (eventData.spanLeft ? (eventData.spanRight ? 'ext-cal-ev-spanboth' : 'ext-cal-ev-spanleft') : (eventData.spanRight ? 'ext-cal-ev-spanright' : ''));
		...
		render : function(config)
		{
			...
			var me = this, spaceChar = '&#160;', weekIndex = 0, eventGrid = config.eventGrid, currentDate = Ext.Date.clone(config.viewStart), currentDateString = '', eventTpl = config.tpl, maxEventsPerDay = config.maxEventsPerDay != undefined ? config.maxEventsPerDay : 999, weekCount = config.weekCount < 1 ? 6 : config.weekCount, dayCount = config.weekCount == 1 ? config.dayCount : 7, eventRow, dayIndex, weekGrid, eventIndex, skippedEventCount, dayGrid, eventCount, currentEvent, cellConfig, eventData;
			...
			for (; weekIndex < weekCount; weekIndex++)
			{
				...
				for (; dayIndex < dayCount; dayIndex++)
				{
					...
					if (weekGrid && weekGrid[dayIndex])
					{
						...
						for (; eventIndex < eventCount; eventIndex++)
						{
							...
							else
							{
								...
								{
									me.renderEvent(currentEvent, weekIndex, dayIndex, eventIndex, dayCount, currentDate, config);
								}
							}
						}
						...
						if (eventCount < config.evtMaxCount[weekIndex])
						{
					...
					currentDate = Extensible.Date.add(currentDate,
					{
						days : 1
					});
				}
...
Ext.define('Extensible.calendar.form.field.CalendarCombo',
{
	...
	{
		return '<div class="x-combo-list-item x-cal-{' + Extensible.calendar.data.CalendarMappings.ColorId.name + '}"><div class="ext-cal-picker-icon">&#160;</div>{' + displayField + '}</div>';
	},
	...
	{
		var me = this, calendarMappings = Extensible.calendar.data.CalendarMappings, colorCls = '', value = me.getValue();
...
Ext.define('Extensible.form.field.DateRangeLayout',
{
	...
	onLayout : function()
	{
		var me = this, shadowCt = me.getShadowCt(), owner = me.owner, singleLine = owner.isSingleLine();

		me.owner.suspendLayout = true;

		if (singleLine)
		{
			shadowCt.getComponent('row1').add(owner.startDate, owner.startTime, owner.toLabel, owner.endTime, owner.endDate, owner.allDay);
		}
		else
		{
...
Ext.define('Extensible.form.field.DateRange',
{
	...
	requires : ['Ext.form.field.Date', 'Ext.form.field.Time', 'Ext.form.Label', 'Ext.form.field.Checkbox'],
	...
	*/
	...
	initComponent : function()
	{
		...
		else
		{
			me.items = [
			{
				...
				items : [me.getStartDateConfig(), me.getStartTimeConfig(), me.getDateSeparatorConfig()]
			},
			{
				xtype : 'container',
				layout : me.fieldLayout,
				items : [me.getEndDateConfig(), me.getEndTimeConfig(), me.getAllDayConfig()]
			}];
		}
	...
	{
		var me = this;
		return [me.getStartDateConfig(), me.getStartTimeConfig(), me.getDateSeparatorConfig(), me.getEndTimeConfig(), me.getEndDateConfig(), me.getAllDayConfig()];
	},

	getLayoutItems : function(singleLine)
	{
		var me = this;
		return singleLine ? me.items.items : [[me.startDate, me.startTime, me.toLabel], [me.endDate, me.endTime, me.allDay]];
	},
	...
	isSingleLine : function()
	{
		...
		if (me.calculatedSingleLine === undefined)
		{
			...
			{
				var ownerCtEl = me.ownerCt.getEl(), w = me.ownerCt.getWidth() - ownerCtEl.getPadding('lr'), el = ownerCtEl.down('.x-panel-body');
...
Ext.define('Extensible.calendar.form.field.ReminderCombo',
{
	...
	{
		var me = this, fmt = me.reminderValueFormat, stringFormat = Ext.String.format;

		return [['', me.noneText], ['0', me.atStartTimeText], ['5', stringFormat(fmt, '5', me.getMinutesText(5))], ['15', stringFormat(fmt, '15', me.getMinutesText(15))], ['30', stringFormat(fmt, '30', me.getMinutesText(30))], ['60', stringFormat(fmt, '1', me.getHoursText(1))], ['90', stringFormat(fmt, '1.5', me.getHoursText(1.5))], ['120', stringFormat(fmt, '2', me.getHoursText(2))], ['180', stringFormat(fmt, '3', me.getHoursText(3))], ['360', stringFormat(fmt, '6', me.getHoursText(6))], ['720', stringFormat(fmt, '12', me.getHoursText(12))], ['1440', stringFormat(fmt, '1', me.getDaysText(1))], ['2880', stringFormat(fmt, '2', me.getDaysText(2))], ['4320', stringFormat(fmt, '3', me.getDaysText(3))], ['5760', stringFormat(fmt, '4', me.getDaysText(4))], ['7200', stringFormat(fmt, '5', me.getDaysText(5))], ['10080', stringFormat(fmt, '1', me.getWeeksText(1))], ['20160', stringFormat(fmt, '2', me.getWeeksText(2))]]
	},
...
Ext.define('Extensible.calendar.util.ColorPicker',
{
	...
	{
		var me = this, selectedCls = me.selectedCls, value = me.value;
...
Ext.define('Extensible.calendar.gadget.CalendarListPanel',
{
	...
	requires : ['Ext.XTemplate', 'Extensible.calendar.gadget.CalendarListMenu'],
	...
	refresh : function()
	{
		...
		var data = [], i = 0, o = null, CM = Extensible.calendar.data.CalendarMappings, recs = this.store.getRange(), len = recs.length;
	...
	{
		var rec = this.store.findRecord(Extensible.calendar.data.CalendarMappings.CalendarId.name, id), CM = Extensible.calendar.data.CalendarMappings, isHidden = rec.data[CM.IsHidden.name];
	...
	{
		var i = 0, recId, calendarId = Extensible.calendar.data.CalendarMappings.CalendarId.name, recs = this.store.getRange(), len = recs.length;
	...
	{
		var id = this.getCalendarId(el.parent('li')), rec = this.store.findRecord(Extensible.calendar.data.CalendarMappings.CalendarId.name, id);
...
Ext.define('Extensible.calendar.menu.Event',
{
	...
	{
		selectedDate = Extensible.Date.copyTime(this.rec.data[Extensible.calendar.data.EventMappings.StartDate.name], selectedDate);
	...
	{
		var me = this, startDate = rec.data[Extensible.calendar.data.EventMappings.StartDate.name];
...
Ext.define('Extensible.calendar.form.EventDetails',
{
	...
	requires : ['Extensible.form.field.DateRange', 'Extensible.calendar.form.field.ReminderCombo', 'Extensible.calendar.data.EventMappings', 'Extensible.calendar.form.field.CalendarCombo', 'Extensible.form.recurrence.Fieldset', 'Ext.layout.container.Column', 'Extensible.form.recurrence.RangeEditWindow'],
	...
	initComponent : function()
	{
		...
		var rightFields = [], leftFields = [this.titleField, this.dateRangeField, this.reminderField, this.notesField, this.locationField, this.urlField];
...
Ext.define('Extensible.calendar.form.EventWindow',
{
	...
	requires : ['Ext.form.Panel', 'Extensible.calendar.data.EventModel', 'Extensible.calendar.data.EventMappings', 'Extensible.form.recurrence.RangeEditWindow'],
	...
	{
		var Field = field.up('form').getComponent(this.id + '-title'), value = field.rawValue;
	...
	{
		var me = this, EventMappings = Extensible.calendar.data.EventMappings, form, rec;

		// Work around the CSS day cell height hack needed for initial render in IE8/strict:
		me.animateTarget = (Ext.isIE8 && Ext.isStrict) ? null : animateTarget;

		me.callParent([me.animateTarget,
		function()
		{
		...
		if (o.data)
		{
			...
			me.formPanel.getComponent(this.id + '-patient_search').hide();
			//GaiaEHR
			me.formPanel.getComponent(this.id + '-title').show();
			//GaiaEHR
		...
		else
		{
			...
			me.formPanel.getComponent(this.id + '-patient_search').show();
			//GaiaEHR
			me.formPanel.getComponent(this.id + '-title').hide();
			//GaiaEHR

			me.setTitle(me.titleTextAdd);

			var start = o[EventMappings.StartDate.name], end = o[EventMappings.EndDate.name] || Extensible.Date.add(start,
			{
				hours : 1
			});
			...
			rec.data[EventMappings.IsAllDay.name] = !!o[EventMappings.IsAllDay.name] || (start.getDate() !== Extensible.Date.add(end,
			{
				millis : 1
			}).getDate());

			rec.data[EventMappings.CalendarId.name] = me.calendarStore ? me.calendarStore.getAt(0).data[Extensible.calendar.data.CalendarMappings.CalendarId.name] : '';

			if (EventMappings.Duration)
			{
				rec.data[EventMappings.Duration.name] = Extensible.Date.diff(start, end, Extensible.calendar.data.EventModel.resolution);
			}
	...
	{
		var fields = record.fields, values = this.formPanel.getForm().getValues(), EventMappings = Extensible.calendar.data.EventMappings, name, obj =
		{
		}, modified;
	...
	onSave : function()
	{
		var me = this, form = me.formPanel.form, originalHasRecurrence = me.activeRecord.isRecurring();
...
Ext.define('Extensible.calendar.view.AbstractCalendar',
{
	...
	requires : ['Ext.CompositeElement', 'Extensible.calendar.form.EventDetails', 'Extensible.calendar.form.EventWindow', 'Extensible.calendar.menu.Event', 'Extensible.calendar.dd.DragZone', 'Extensible.calendar.dd.DropZone', 'Extensible.form.recurrence.RangeEditWindow'],
	...
	forceSize : function()
	{
		...
		if (el && el.down)
		{
			var hd = el.down('.ext-cal-hd-ct'), bd = el.down('.ext-cal-body-ct');
			...
			var headerHeight = hd.getHeight(), sz = el.parent().getSize();
	...
	{
		var lastInMonth = Ext.Date.getLastDateOfMonth(this.startDate), w = 0, d = 0, row = 0, currentDt = Ext.Date.clone(this.viewStart), weeks = this.weekCount < 1 ? 6 : this.weekCount;
		...
		{
			var EventMappings = Extensible.calendar.data.EventMappings, startDt = Ext.Date.clearTime(rec.data[EventMappings.StartDate.name], true), startsOnDate = currentDt.getTime() === startDt.getTime(), spansFromPrevView = (w === 0 && d === 0 && (currentDt > rec.data[EventMappings.StartDate.name]));
		...
		for (; w < weeks; w++)
		{
			...
			for ( d = 0; d < this.dayCount; d++)
			{
				...
				currentDt = Extensible.Date.add(currentDt,
				{
					days : 1
				});
			}
	...
	{
		var me = this, row = 0, dt = Ext.Date.clone(me.viewStart), maxEventsForDay;
		...
		evts.each(function(evt)
		{
			...
			{
				var daysInView = Extensible.Date.diffDays(Extensible.Date.max(me.viewStart, evt.data[M.StartDate.name]), Extensible.Date.min(me.viewEnd, evt.data[M.EndDate.name])) + 1;
	...
	prepareEventGridSpans : function(evt, grid, w, d, days, allday)
	{
		...
		var w1 = w, d1 = d, row = this.findEmptyRowIndex(w, d, allday), dt = Ext.Date.clone(this.viewStart);
	...
	{
		var grid = allday ? this.allDayGrid : this.eventGrid, day = grid[w] ? grid[w][d] || [] : [], i = 0, len = day.length;
	...
	onCalendarEndDrag : function(start, end, onComplete)
	{
		...
		if (this.fireEvent('rangeselect', this, dates, boundOnComplete) !== false)
		{
			...
			if (this.editWin)
			{
				this.editWin.on('hide', boundOnComplete, this,
				{
					single : true
				});
			}
	...
	onRemove : function(store, operation)
	{
		...
		var reloadRequired = this.storeReloadRequired('delete', operation), rec = operation.records[0];
	...
	{
		var prefix = forSelect ? '.' : '', id = this.getEventId(eventId), cls = prefix + this.id + this.eventElIdDelimiter + id;
	...
	{
		var M = Extensible.calendar.data.EventMappings, data = evt.data || evt, calRec = this.calendarStore ? this.calendarStore.findRecord(M.CalendarId.name, evt[M.CalendarId.name]) : null;
		...
		var start = this.viewStart.getTime(), end = this.viewEnd.getTime(), evStart = data[M.StartDate.name].getTime(), evEnd = data[M.EndDate.name].getTime();
	...
	{
		var ev1 = evt1.data ? evt1.data : evt1, ev2 = evt2.data ? evt2.data : evt2, M = Extensible.calendar.data.EventMappings, start1 = ev1[M.StartDate.name].getTime(), end1 = Extensible.Date.add(ev1[M.EndDate.name],
		{
			seconds : -1
		}).getTime(), start2 = ev2[M.StartDate.name].getTime(), end2 = Extensible.Date.add(ev2[M.EndDate.name],
		{
			seconds : -1
		}).getTime(), startDiff = Extensible.Date.diff(ev1[M.StartDate.name], ev2[M.StartDate.name], 'm');
		...
		var evtsOverlap = Extensible.Date.rangesOverlap(start1, end1, start2, end2), minimumMinutes = this.minEventDisplayMinutes || 0, // applies in day/week body view only for vertical overlap
	...
	{
		var M = Extensible.calendar.data.EventMappings, data = evt.data || evt, diff;
	...
	setStartDate : function(start, /*private*/reload)
	{
		...
		var cloneDt = Ext.Date.clone, cloneStartDate = me.startDate ? cloneDt(me.startDate) : null, cloneStart = cloneDt(start), cloneViewStart = me.viewStart ? cloneDt(me.viewStart) : null, cloneViewEnd = me.viewEnd ? cloneDt(me.viewEnd) : null;
	...
	{
		var me = this, start = startDate || me.startDate, offset = start.getDay() - me.startDay, Dt = Extensible.Date;
		...
		switch(this.weekCount)
		{
			case 0:
			case 1:
				me.viewStart = me.dayCount < 7 && !me.startDayIsStatic ? start : Dt.add(start,
				{
					days : -offset,
					clearTime : true
				});
	...
	sortEventRecordsForDay : function(evts)
	{
		...
		{
			var a = evtA.data, b = evtB.data, M = Extensible.calendar.data.EventMappings;
	...
	{
		Ext.Msg.alert(this.notifyOnExceptionTitle, this.notifyOnExceptionText + '<br>' + this.getExceptionMessage(response));
	},
	...
	{
		var idx = this.store.find(Extensible.calendar.data.EventMappings.EventId.name, id, 0, // start index
	...
	onDayClick : function(dt, ad, el)
	{
		...
		{
			var M = Extensible.calendar.data.EventMappings, data =
			{
			};
	...
	{
		var EventMappings = Extensible.calendar.data.EventMappings, diff = newStartDate.getTime() - rec.getStartDate().getTime(), updateData =
		{
		};

		updateData[EventMappings.StartDate.name] = newStartDate;
		updateData[EventMappings.EndDate.name] = Extensible.Date.add(rec.getEndDate(),
		{
			millis : diff
		});
	...
	{
		var el = e.getTarget(this.eventSelector, 5, true), match = false;
	...
	onClick : function(e, t)
	{
		var me = this, el = e.getTarget(me.eventSelector, 5);
		...
		if (el)
		{
			var id = me.getEventIdFromEl(el), rec = me.getEventRecord(id);
	...
	handleDayMouseEvent : function(e, t, type)
	{
		...
		if (t)
		{
			...
			{
				var dt = this.getDateFromId(t.id, this.dayElIdDelimiter), rel = Ext.get(e.getRelatedTarget()), relTD, relDate;
...
Ext.define('Extensible.calendar.view.MonthDayDetail',
{
	...
	requires : ['Ext.XTemplate', 'Extensible.calendar.view.AbstractCalendar'],
	...
	refresh : function()
	{
		...
		{
			var thisDt = Ext.Date.clearTime(this.date, true).getTime(), M = Extensible.calendar.data.EventMappings, recStart = Ext.Date.clearTime(rec.data[M.StartDate.name], true).getTime(), startsOnDate = (thisDt == recStart), spansDate = false, calId = rec.data[M.CalendarId.name], calRec = this.calendarStore ? this.calendarStore.getById(calId) : null;
		...
		evts.each(function(evt)
		{
			var item = evt.data, M = Extensible.calendar.data.EventMappings;
...
Ext.define('Extensible.calendar.view.Month',
{
	...
	requires : ['Ext.XTemplate', 'Extensible.calendar.template.Month', 'Extensible.calendar.util.WeekEventRenderer', 'Extensible.calendar.view.MonthDayDetail'],
	...
	forceSize : function()
	{
		...
		{
			var hd = this.el.down('.ext-cal-hd-days-tbl'), bgTbl = this.el.select('.ext-cal-bg-tbl'), evTbl = this.el.select('.ext-cal-evt-tbl'), wkLinkW = this.el.down('.ext-cal-week-link').getWidth(), w = this.el.getWidth() - wkLinkW;
	...
	getEventTemplate : function()
	{
		if (!this.eventTpl)
		{
			...
			tpl = !(Ext.isIE || Ext.isOpera) ? Ext.create('Ext.XTemplate', '<div class="{_extraCls} {spanCls} ext-cal-evt ext-cal-evr">', body, '</div>') : Ext.create('Ext.XTemplate', '<tpl if="_renderAsAllDay">', '<div class="{_extraCls} {spanCls} ext-cal-evt ext-cal-evo">', '<div class="ext-cal-evm">', '<div class="ext-cal-evi">', '</tpl>', '<tpl if="!_renderAsAllDay">', '<div class="{_extraCls} ext-cal-evt ext-cal-evr">', '</tpl>', body, '<tpl if="_renderAsAllDay">', '</div>', '</div>', '</tpl>', '</div>');
			tpl.compile();
			this.eventTpl = tpl;
		}
	...
	getTemplateEventData : function(evt)
	{
		var M = Extensible.calendar.data.EventMappings, extraClasses = [this.getEventSelectorCls(evt[M.EventId.name])], data =
		{
		}, recurring = evt[M.RRule.name] != '', colorCls = 'x-cal-default', title = evt[M.Title.name], fmt = Extensible.Date.use24HourTime ? 'G:i ' : 'g:ia ';
		...
		{
			var rec = this.getEventRecord(evt[M.EventId.name]), cls = this.getEventClass(rec, !!evt._renderAsAllDay, data, this.store);
		...
		data.Title = (evt[M.IsAllDay.name] ? '' : Ext.Date.format(evt[M.StartDate.name], fmt)) + (!title || title.length == 0 ? this.defaultEventTitleText : title);
	...
	{
		var box = this.el.getBox(), padding = this.getViewPadding(), w = (box.width - padding.width) / this.dayCount, h = (box.height - padding.height) / this.getWeekCount();
	...
	getEventHeight : function()
	{
		if (!this.eventHeight)
		{
			...
			{
				return 16;
				// no events rendered, so try setting this.eventHeight again later
			}
		}
	...
	{
		var dayHeight = this.getDaySize(true).height, eventHeight = this.getEventHeight(), max = Math.max(Math.floor((dayHeight - eventHeight) / eventHeight), 0);
	...
	{
		var sides = sides || 'tlbr', top = sides.indexOf('t') > -1, left = sides.indexOf('l') > -1, right = sides.indexOf('r') > -1, height = this.showHeader && top ? this.el.select('.ext-cal-hd-days-tbl').first().getHeight() : 0, width = 0;
	...
	{
		var box = this.el.getBox(), padding = this.getViewPadding('tl'), // top/left only since we only want the xy offsets
		daySize = this.getDaySize(), dayL = Math.floor(((x - box.x - padding.width) / daySize.width)), dayT = Math.floor(((y - box.y - padding.height) / daySize.height)), days = (dayT * 7) + dayL, dt = Extensible.Date.add(this.viewStart,
		{
			days : days
		});
	...
	{
		var p = this.detailPanel, dayEl = this.getDayEl(dt), box = dayEl.getBox(), innerTplHeight = p.el.down('.ext-cal-mdv').getHeight(), header = p.getDockedItems('header')[0], frameSize = p.frameSize ||
		{
			top : 0,
			bottom : 0
		}, frameHeight = frameSize.top + frameSize.bottom + header.getHeight(), bodyHeight = innerTplHeight + frameHeight + 5, documentBodyHeight = Ext.getBody().getHeight() - 20, calculatedHeight = Math.min(bodyHeight, documentBodyHeight);
	...
	onClick : function(e, t)
	{
		...
		if ( el = e.getTarget('td', 3))
		{
			...
			{
				var parts = el.id.split(this.dayElIdDelimiter), dt = parts[parts.length - 1];
...
Ext.define('Extensible.calendar.view.DayHeader',
{
	...
	requires : ['Extensible.calendar.template.DayHeader'],
...
Ext.define('Extensible.calendar.view.DayBody',
{
	...
	requires : ['Ext.XTemplate', 'Extensible.calendar.template.DayBody', 'Extensible.calendar.data.EventMappings', 'Extensible.calendar.dd.DayDragZone', 'Extensible.calendar.dd.DayDropZone'],
	...
	afterRender : function()
	{
		...
		var startHour = Math.max(this.scrollStartHour, this.viewStartHour), scrollStart = Math.max(0, startHour - this.viewStartHour);
	...
	{
		var me = this, EventMappings = Extensible.calendar.data.EventMappings, compareFn = Extensible.Date.compare;

		if (compareFn(rec.getStartDate(), data[EventMappings.StartDate.name]) === 0 && compareFn(rec.getEndDate(), data[EventMappings.EndDate.name]) === 0)
		{
			// no changes
			return;
		}
	...
	{
		var EventMappings = Extensible.calendar.data.EventMappings, startDateName = EventMappings.StartDate.name, endDateName = EventMappings.EndDate.name, updateData =
		{
		};
		...
		if (EventMappings.Duration)
		{
			updateData[EventMappings.Duration.name] = Extensible.Date.diff(data[startDateName], data[endDateName], Extensible.calendar.data.EventModel.resolution);
		}
	...
	getEventBodyMarkup : function()
	{
		if (!this.eventBodyMarkup)
		{
			this.eventBodyMarkup = ['{Title}', '<tpl if="_isReminder">', '<i class="ext-cal-ic ext-cal-ic-rem">&#160;</i>', '</tpl>', '<tpl if="_isRecurring">', '<i class="ext-cal-ic ext-cal-ic-rcr">&#160;</i>', '</tpl>'
			//                '<tpl if="spanLeft">',
	...
	getTemplateEventData : function(evt)
	{
		var M = Extensible.calendar.data.EventMappings, extraClasses = [this.getEventSelectorCls(evt[M.EventId.name])], data =
		{
		}, colorCls = 'x-cal-default', title = evt[M.Title.name], fmt = Extensible.Date.use24HourTime ? 'G:i ' : 'g:ia ', recurring = evt[M.RRule.name] !== '', rec;

		this.getTemplateEventBox(evt);

		if (this.calendarStore && evt[M.CalendarId.name])
		{
			rec = this.calendarStore.findRecord(Extensible.calendar.data.CalendarMappings.CalendarId.name, evt[M.CalendarId.name]);
	...
	{
		var heightFactor = this.hourHeight / this.hourIncrement, start = evt[Extensible.calendar.data.EventMappings.StartDate.name], end = evt[Extensible.calendar.data.EventMappings.EndDate.name], startOffset = Math.max(start.getHours() - this.viewStartHour, 0), endOffset = Math.min(end.getHours() - this.viewStartHour, this.viewEndHour - this.viewStartHour), startMins = startOffset * this.hourIncrement, endMins = endOffset * this.hourIncrement, viewEndDt = Extensible.Date.add(Ext.Date.clone(end),
		{
			hours : this.viewEndHour,
			clearTime : true
	...
	renderItems : function()
	{
		...
		{
			var ev = 0, emptyCells = 0, skipped = 0, d = this.eventGrid[0][day], ct = d ? d.length : 0;
			...
			for (; ev < ct; ev++)
			{
				...
				var item = evt.data || evt.event.data, M = Extensible.calendar.data.EventMappings, ad = item[M.IsAllDay.name] === true, span = this.isEventSpanning(evt.event || evt), renderAsAllDay = ad || span;
				...
				evts.push(
				{
					...
					date : Extensible.Date.add(this.viewStart,
					{
						days : day
					})
				});
		...
		var i = 0, j = 0, overlapCols = [], l = evts.length, prevDt, evt2, dt;
		...
		for ( i = 0; i < l; i++)
		{
			...
			{
				var colWidth = 100 / (overlapCols[dt] + 1), evtWidth = 100 - (colWidth * evt._overlap);

				evt._width = colWidth;
				evt._left = colWidth * evt._overcol;
			}
			var markup = this.getEventTemplate().apply(evt), target = this.id + '-day-col-' + Ext.Date.format(evts[i].date, 'Ymd');
	...
	getDayAt : function(x, y)
	{
		var sel = '.ext-cal-body-ct', xoffset = this.el.down('.ext-cal-day-times').getWidth(), viewBox = this.el.getBox(), daySize = this.getDaySize(false), relX = x - viewBox.x - xoffset, dayIndex = Math.floor(relX / daySize.width), // clicked col index
		scroll = this.el.getScroll(), row = this.el.down('.ext-cal-bg-row'), // first avail row, just to calc size
		rowH = row.getHeight() / this.incrementsPerHour, relY = y - viewBox.y - rowH + scroll.top, rowIndex = Math.max(0, Math.ceil(relY / rowH)), mins = rowIndex * (this.hourIncrement / this.incrementsPerHour), dt = Extensible.Date.add(this.viewStart,
		{
			days : dayIndex,
			minutes : mins,
			hours : this.viewStartHour
...
Ext.define('Extensible.calendar.view.Day',
{
	...
	requires : ['Extensible.calendar.view.AbstractCalendar', 'Extensible.calendar.view.DayHeader', 'Extensible.calendar.view.DayBody'],
	...
	forceSize : function()
	{
		...
		Ext.defer(function()
		{
			var ct = me.el.up('.x-panel-body'), header = me.el.down('.ext-cal-day-header'), bodyHeight = ct ? ct.getHeight() - header.getHeight() : false;
...
Ext.define('Extensible.calendar.CalendarPanel',
{
	...
	requires : ['Ext.layout.container.Card', 'Extensible.calendar.view.Day', 'Extensible.calendar.view.Week', 'Extensible.calendar.view.Month', 'Extensible.calendar.view.MultiDay', 'Extensible.calendar.view.MultiWeek'],
	...
	initComponent : function()
	{
		...
		var text, multiDayViewCount = (this.multiDayViewCfg && this.multiDayViewCfg.dayCount) || 3, multiWeekViewCount = (this.multiWeekViewCfg && this.multiWeekViewCfg.weekCount) || 2;
	...
	initEventRelay : function(cfg)
	{
		...
		cfg.listeners.afterrender =
		{
			fn : function(c)
			{
				...
				this.relayEvents(c, ['eventsrendered', 'eventclick', 'dayclick', 'eventover', 'eventout', 'beforedatechange', 'datechange', 'rangeselect', 'beforeeventcopy', 'eventcopy', 'beforeeventmove', 'eventmove', 'initdrag', 'dayover', 'dayout', 'beforeeventresize', 'eventresize', 'eventadd', 'eventupdate', 'beforeeventdelete', 'eventdelete', 'eventcancel', 'eventexception']);
	...
	{
		var me = this, layout = me.layout, editViewId = me.id + '-edit', toolbar;
	...
	fireViewChange : function()
	{
		...
		{
			var view = this.layout.getActiveItem(), cloneDt = Ext.Date.clone;
	...
	{
		var me = this, activeItem = me.layout.activeItem;

		if (activeItem && me.showNavBar !== false)
		{
			var suffix = activeItem.id.split(me.id + '-')[1], btn = Ext.getCmp(me.id + '-tb-' + suffix);
