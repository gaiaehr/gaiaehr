	...
	public function updateService(stdClass $params) {
		try {
			...
			if($params->code_type == 'CPT4'){
				...
				$sql = $this->db->sqlBind($data, 'cpt_codes', 'U', ['id' => $params->id]);
			} elseif($params->code_type == 'HCPCS') {
				$sql = $this->db->sqlBind($data, 'hcpcs_codes', 'U', ['id' => $params->id]);
			} elseif($params->code_type == 'Immunizations') {
				$sql = $this->db->sqlBind($data, 'immunizations', 'U', ['id' => $params->id]);
			} else {
