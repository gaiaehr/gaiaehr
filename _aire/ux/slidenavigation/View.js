...
Ext.define('Ext.ux.slidenavigation.View', {
    ...
    addItems: function(data) {
        var me = this,
            items = Ext.isArray(data) ? data : [data];
    ...
    closeContainer: function(duration) {
        var d = duration || this.config.slideDuration;
        this.moveContainer(0, d);
    },
    ...
    openContainer: function(duration) {
        ...
        this.moveContainer(this.config.list.width, d);
    },
    
    toggleContainer: function(duration) {
        var d = Ext.isNumber(duration) ? duration : this.config.slideDuration;
        if (this.isClosed()) {
            this.openContainer(d);
        } else {
            this.closeContainer(d);
        }
    },
    ...
    createContainer: function() {
        return Ext.create('Ext.Container', Ext.merge({}, this.config.container, {
            ...
            draggable: {
                ...
                constraint: {
                    min: { x: this.config.list.minDrag || 0, y: 0 },
