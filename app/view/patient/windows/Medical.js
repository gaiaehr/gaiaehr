...
Ext.define('App.view.patient.windows.Medical',
{
	...
	{
		var form = editor.editor.getForm(), search = form.findField('immunization_id'), name = form.findField('immunization_name'), newRecord = e.record.data.immunization_name == '';
	...
	onLiveSearchSelect : function(combo, model)
	{
		...
		if (combo.action == 'immunization_id')
		{
			...
			field.setValue(name);
			me.CvxMvxCombo.store.load(
			{
				params :
				{
					cvx_code : model[0].data.cvx_code
				}
			})
		}
		else
	...
	{
		var me = this, skinCombo = me.allergiesSkin, localCombo = me.allergiesLocal, abdominalCombo = me.allergiesAbdominal, systemicCombo = me.allergiesSystemic, value = combo.getValue();
	...
	onAllergyTypeSelect : function(combo, record)
	{
		...
		if (record[0].data.allergy_type == 'Food')
		{
			me.allergieType.reset();
		...
		if (record[0].data.allergy_type == 'Environmental')
		{
			me.allergieType.reset();
			me.allergieType.show(true);
			me.allergieType.setDisabled(false);
