...
class AutomatedMeasureCalculation extends Reports{
    ...
    function getProblemListMeasure($Parameters = null, $Stage = '1'){
        try{
            ...
            // Consume Parameters
            $begin_date = $Parameters['begin_date'];    // YYYY-MM-DD
            $end_date = $Parameters['end_date'];        // YYYY-MM-DD
                ...
                case '1':
                    $SQL = "SELECT *, ROUND((NUME/DENOM*100)) AS PERCENT
	                    ...
	                    (SELECT count(distinct(patient.pid)) as NUME
		                    ...
		                    AND encounters.provider_uid = $provider_id
		                    INNER JOIN patient_medications ON patient.pid = patient_medications.pid
		                    AND patient_medications.date_ordered BETWEEN '$begin_date' AND '$end_date') AS HAVINGMEDORDERS";
                    break;
                case '2':
                case '2A':
                case '1A':
                ...
                default:
                    $SQL ="SELECT *, ROUND((NUME/DENOM*100)) AS PERCENT
                        ...
                        (SELECT count(patient_orders.pid) AS DENOM
		                    FROM patient_orders
		                    INNER JOIN encounters ON patient_orders.pid = encounters.pid
		                    AND encounters.provider_uid = $provider_id
		                    WHERE patient_orders.date_ordered BETWEEN '$begin_date' AND '$end_date'
                            AND patient_orders.order_type = 'lab') AS LABORDERS,
	                    (SELECT count(patient_orders.pid) as NUME
		                    FROM patient_orders
		                    INNER JOIN encounters ON patient_orders.pid = encounters.pid
		                    AND encounters.provider_uid = $provider_id
		                    WHERE patient_orders.priority != ''
		                    AND patient_orders.date_ordered BETWEEN '$begin_date' AND '$end_date'
		                    AND patient_orders.order_type = 'lab') AS LABORDERSCPOE";
                    break;
            }
        } catch(\Exception $Error) {
            return $Error;
    ...
    function getCPOEMeasure_Radiology($Parameters = null, $Stage = '2'){
        try{
            ...
            // Validation
            if(!isset($Parameters))
                throw new \Exception('No parameters provided for CPOE Radiology Measure');
            if(!isset($Stage))
                throw new \Exception('No Stage provided for CPOE Radiology Measure');
            if(!isset($Parameters['begin_date']))
                throw new \Exception('No [begin_date] parameter provided for CPOE Radiology Measure');
            if(!isset($Parameters['end_date']))
                throw new \Exception('No [end_date] parameter provided for CPOE Radiology Measure');
            if(!isset($Parameters['provider_id']))
                throw new \Exception('No [provider_id] parameter provided for CPOE Radiology Measure');
                    ...
                    $SQL ="SELECT *, ROUND((NUME/DENOM*100)) AS PERCENT
                        FROM
                            (SELECT count(patient_orders.pid) AS DENOM
		                        FROM patient_orders
		                        WHERE patient_orders.date_ordered BETWEEN '$begin_date' AND '$end_date'
		                        AND patient_orders.uid = $provider_id) AS UNIQUEORDERS,
	                        (SELECT count(patient_orders.pid) as NUME
		                        FROM patient_orders
    ...
    function getVitalSignsMeasure($Parameters = null, $Stage = '2'){
        try{
            ...
            // Validation
            if(!isset($Parameters))
                throw new \Exception('No parameters provided for Vital Signs Measure');
            if(!isset($Stage))
                throw new \Exception('No Stage provided for Vital Signs Measure');
            if(!isset($Parameters['begin_date']))
                throw new \Exception('No [begin_date] parameter provided for Vital Signs Measure');
            if(!isset($Parameters['end_date']))
                throw new \Exception('No [end_date] parameter provided for Vital Signs Measure');
            if(!isset($Parameters['provider_id']))
                throw new \Exception('No [provider_id] parameter provided for Vital Signs Measure');
                ...
                case '12HTWTOUT':
                    $SQL = "SELECT *, ROUND((NUME/DENOM*100)) AS PERCENT
                        ...
                        (SELECT count(distinct(patient.pid)) AS DENOM
		                    ...
		                    AND encounters.service_date BETWEEN '$begin_date' AND '$end_date'
		                    AND encounters.provider_uid = $provider_id
		                    WHERE (DATE_FORMAT(FROM_DAYS(TO_DAYS(NOW())-TO_DAYS(patient.DOB)), '%Y') + 0) >= 3) AS UNIQUEPATIENTS,
	                    (SELECT count(distinct(patient.pid)) as NUME
		                    FROM patient
		                    INNER JOIN encounters ON patient.pid = encounters.pid
		                    AND encounters.service_date BETWEEN '$begin_date' AND '$end_date'
		                    AND encounters.provider_uid = $provider_id
		                    INNER JOIN encounter_vitals ON patient.pid = encounter_vitals.pid
		                    AND encounter_vitals.bp_systolic IS NOT NULL
		                    AND encounter_vitals.bp_diastolic IS NOT NULL
		                    WHERE (DATE_FORMAT(FROM_DAYS(TO_DAYS(NOW())-TO_DAYS(patient.DOB)), '%Y') + 0) >= 3) AS HAVINGVITALS";
                    break;
                case '12BPOUT':
                    $SQL = "SELECT *, ROUND((NUME/DENOM*100)) AS PERCENT
                        FROM
	                    ...
	                    (SELECT count(distinct(patient.pid)) as NUME
		                    ...
		                    AND encounters.provider_uid = $provider_id
		                    INNER JOIN encounter_vitals ON patient.pid = encounter_vitals.pid
		                    AND encounter_vitals.height_in IS NOT NULL
		                    AND encounter_vitals.height_cm IS NOT NULL
                            AND encounter_vitals.weight_kg IS NOT NULL
                            AND encounter_vitals.weight_lbs IS NOT NULL) AS HAVINGVITALS";
                    break;
                case '12A':
                    $SQL = "SELECT *, ROUND((NUME/DENOM*100)) AS PERCENT
	                    FROM
	                    ...
	                    (SELECT count(distinct(patient.pid)) as NUME
		                    ...
		                    INNER JOIN encounter_vitals ON patient.pid = encounter_vitals.pid
		                    AND
		                    (encounter_vitals.height_in IS NOT NULL
		                    AND encounter_vitals.height_cm IS NOT NULL
		                    AND encounter_vitals.weight_kg IS NOT NULL
		                    AND encounter_vitals.weight_lbs IS NOT NULL
		                    AND encounter_vitals.bp_systolic IS NOT NULL
		                    AND encounter_vitals.bp_diastolic IS NOT NULL
		                    AND (DATE_FORMAT(FROM_DAYS(TO_DAYS(NOW())-TO_DAYS(patient.DOB)), '%Y') + 0) >= 3)
		                    OR
		                    (encounter_vitals.height_in IS NOT NULL
		                    AND encounter_vitals.height_cm IS NOT NULL
		                    AND encounter_vitals.weight_kg IS NOT NULL
		                    AND encounter_vitals.weight_lbs IS NOT NULL
		                    AND (DATE_FORMAT(FROM_DAYS(TO_DAYS(NOW())-TO_DAYS(patient.DOB)), '%Y') + 0) <= 3)) AS HAVINGVITALS";
                    break;
                case '1':
                    $SQL = "SELECT *, ROUND((NUME/DENOM*100)) AS PERCENT
	                    FROM
                        (SELECT count(distinct(patient.pid)) AS DENOM
		                    ...
		                    AND encounters.service_date BETWEEN '$begin_date' AND '$end_date'
		                    AND encounters.provider_uid = $provider_id
                            AND (DATE_FORMAT(FROM_DAYS(TO_DAYS(NOW())-TO_DAYS(patient.DOB)), '%Y') + 0) >= 2) AS UNIQUEPATIENTS,
	                    (SELECT count(distinct(patient.pid)) as NUME
		                    FROM patient
		                    INNER JOIN encounters ON patient.pid = encounters.pid
		                    AND encounters.service_date BETWEEN '$begin_date' AND '$end_date'
		                    ...
		                    AND encounter_vitals.bp_systolic IS NOT NULL
		                    AND encounter_vitals.bp_diastolic IS NOT NULL
		                    WHERE (DATE_FORMAT(FROM_DAYS(TO_DAYS(NOW())-TO_DAYS(patient.DOB)), '%Y') + 0) >= 2) AS HAVINGVITALS";
                    break;
            }
        } catch(\Exception $Error) {
            return $Error;
    ...
    function getSmokingStatusMeasure($Parameters = null, $Stage = '2'){
        try{
            ...
            // Validation
            if(!isset($Parameters))
                throw new \Exception('No parameters provided for Smoking Status Measure');
            if(!isset($Stage))
                throw new \Exception('No Stage provided for Smoking Status Measure');
            if(!isset($Parameters['begin_date']))
                throw new \Exception('No [begin_date] parameter provided for Smoking Status Measure');
            if(!isset($Parameters['end_date']))
                throw new \Exception('No [end_date] parameter provided for Smoking Status Measure');
            if(!isset($Parameters['provider_id']))
                throw new \Exception('No [provider_id] parameter provided for Smoking Status Measure');
    ...
    function getLabResultsMeasure($Parameters = null, $Stage = '2'){
        try{
            ...
            // Validation
            if(!isset($Parameters))
                throw new \Exception('No parameters provided for Lab Results Measure');
            if(!isset($Stage))
                throw new \Exception('No Stage provided for Lab Results Measure');
            if(!isset($Parameters['begin_date']))
                throw new \Exception('No [begin_date] parameter provided for Lab Results Measure');
            if(!isset($Parameters['end_date']))
                throw new \Exception('No [end_date] parameter provided for Lab Results Measure');
            if(!isset($Parameters['provider_id']))
                throw new \Exception('No [provider_id] parameter provided for Lab Results Measure');
            ...
            switch($Stage){
                default:
                    $SQL = "SELECT *, ROUND((NUME/DENOM*100)) AS PERCENT
                        ...
                        (SELECT count(distinct(patient.pid)) AS DENOM
		                    FROM patient
		                    INNER JOIN patient_orders ON patient.pid = patient_orders.pid
		                    AND patient_orders.order_type='lab'
		                    INNER JOIN encounters ON patient.pid = encounters.pid
		                    AND encounters.service_date BETWEEN '$begin_date' AND '$end_date'
		                    AND encounters.provider_uid = $provider_id) AS HAVINGORDERS,
	                    (SELECT count(distinct(patient.pid)) as NUME
		                    FROM patient
		                    INNER JOIN encounters ON patient.pid = encounters.pid
		                    AND encounters.service_date BETWEEN '$begin_date' AND '$end_date'
		                    ...
		                    INNER JOIN patient_orders ON patient.pid = patient_orders.pid
		                    AND patient_orders.order_type='lab'
		                    INNER JOIN patient_order_results ON patient_orders.id = patient_order_results.order_id) AS HAVINGRSULTS";
                    break;
            }
        } catch(\Exception $Error) {
            return $Error;
    ...
    function getPatientRemindersMeasure($Parameters = null, $Stage = '2'){
        try{
            ...
            // Validation
            if(!isset($Parameters))
                throw new \Exception('No parameters provided for Patient Reminders Measure');
            if(!isset($Stage))
                throw new \Exception('No Stage provided for Patient Reminders Measure');
            if(!isset($Parameters['begin_date']))
                throw new \Exception('No [begin_date] parameter provided for Patient Reminders Measure');
            if(!isset($Parameters['end_date']))
                throw new \Exception('No [end_date] parameter provided for Patient Reminders Measure');
            if(!isset($Parameters['provider_id']))
                throw new \Exception('No [provider_id] parameter provided for Patient Reminders Measure');
                ...
                case '1':
                    $SQL = "SELECT *, ROUND((NUME/DENOM*100)) AS PERCENT
                        ...
                        (SELECT count(distinct(patient.pid)) AS DENOM
		                    ...
		                    INNER JOIN encounters ON patient.pid = encounters.pid
		                    AND encounters.provider_uid = $provider_id
		                    WHERE ((DATE_FORMAT(FROM_DAYS(TO_DAYS(NOW())-TO_DAYS(patient.DOB)), '%Y') + 0) >= 65 OR
                            (DATE_FORMAT(FROM_DAYS(TO_DAYS(NOW())-TO_DAYS(patient.DOB)), '%Y') + 0) <= 5)) AS HAVINGPATIENTS,
	                    (SELECT count(distinct(patient.pid)) as NUME
		                    FROM patient
		                    INNER JOIN encounters ON patient.pid = encounters.pid
		                    AND encounters.service_date BETWEEN '$begin_date' AND '$end_date'
		                    AND encounters.provider_uid = $provider_id
		                    INNER JOIN patient_reminders ON patient.pid = patient_reminders.pid
		                    WHERE ((DATE_FORMAT(FROM_DAYS(TO_DAYS(NOW())-TO_DAYS(patient.DOB)), '%Y') + 0) >= 65 OR
                            (DATE_FORMAT(FROM_DAYS(TO_DAYS(NOW())-TO_DAYS(patient.DOB)), '%Y') + 0) <= 5)) AS HAVINGREMINDERS";
                    break;
                case '2':
                    $SQL = "SELECT *, ROUND((NUME/DENOM*100)) AS PERCENT
	                    FROM
                        (SELECT count(distinct(patient.pid)) AS DENOM
		                    ...
		                    INNER JOIN encounters ON patient.pid = encounters.pid
		                    AND encounters.provider_uid = $provider_id
		                    AND encounters.service_date BETWEEN DATE_SUB('$begin_date', INTERVAL 24 MONTH) AND '$end_date'
		                    WHERE (SELECT count(*) FROM encounters WHERE encounters.pid = patient.pid) >= 2) AS HAVINGPATIENTS,
	                    (SELECT count(distinct(patient.pid)) as NUME
		                    FROM patient
		                    INNER JOIN encounters ON patient.pid = encounters.pid
                            AND encounters.provider_uid = $provider_id
		                    AND encounters.service_date BETWEEN DATE_SUB('$begin_date', INTERVAL 24 MONTH) AND '$end_date'
		                    AND (SELECT count(*) FROM encounters WHERE encounters.pid = patient.pid) >= 2
		                    INNER JOIN patient_reminders ON patient.pid = patient_reminders.pid) AS HAVINGREMINDERS";
                    break;
            }
        } catch(\Exception $Error) {
            return $Error;
