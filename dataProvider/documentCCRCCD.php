...
class CCD
{
	...
	{
		$this->CDAdocument  = new DOMDocument('1.0', 'UTF-8');
    ...
    /**
     * XMLData function
     * Create XML data for CCR or CCD XSL stylesheet/template document
     */
    function XMLData($request)
    {
         ...
         * Header
         */
        $this->createHeader($CDA);
        $XMLDataDocument = $this->CDAdocument->createElement('Body');
        $CDA->appendChild($XMLDataDocument);
         ...
         * Problems
         */
        $e_Problems = $this->CDAdocument->createElement('Problems');
        $this->createProblem($e_Problems);
        $XMLDataDocument->appendChild($e_Problems);

        /**
         * Alerts
         */
        $e_Alerts = $this->CDAdocument->createElement('Alerts');
        $this->createAlerts($e_Alerts);
        $XMLDataDocument->appendChild($e_Alerts);

        /**
         * Medication
         */
        $e_Medications = $this->CDAdocument->createElement('Medications');
        $this->createMedications($e_Medications);
        $XMLDataDocument->appendChild($e_Medications);

        /**
         * Immunization
         */
        $e_Immunizations = $this->CDAdocument->createElement('Immunizations');
        $this->createImmunizations($e_Immunizations);
        $XMLDataDocument->appendChild($e_Immunizations);

        /**
         * Results
         */
        $e_Results = $this->CDAdocument->createElement('Results');
        $this->createResults($e_Results);
        $XMLDataDocument->appendChild($e_Results);
	...
	function createCCD($request)
	{
		...
		$e_styleSheet = $this->CDAdocument->createProcessingInstruction('xml-stylesheet', 'type="text/xsl" href="'.$_SESSION['url'].'/lib/ccr/stylesheet/ccr.xsl"');
		$this->CDAdocument->appendChild($e_styleSheet);

		$e_ccr = $this->CDAdocument->createElementNS('urn:astm-org:CCR', 'ContinuityOfCareRecord');
		$this->CDAdocument->appendChild($e_ccr);

		/**
		 * Header
		 */
		$this->createHeader($e_ccr);
		$e_Body = $this->CDAdocument->createElement('Body');
    ...
    function generateCCR($raw)
	{
        ...
        else
        {
			if($raw == 'hybrid')
            {
				...
				$this->createHybridXML($this->CDAdocument);
			}
            else
            {
				if($raw == 'pure')
	...
	{
		$e_Source = $this->CDAdocument->createElement('Source');
		$e_Actor  = $this->CDAdocument->createElement('Actor');
		$e_Source->appendChild($e_Actor);
		$e_ActorID = $this->CDAdocument->createElement('ActorID', $uuid);
	...
	{
		// save the raw xml
		$main_xml = $this->CDAdocument->saveXml();
		// save the stylesheet
	...
	function createProblem($e_Problems)
	{
        ...
        {
			$pCount++;
			$e_Problem = $this->CDAdocument->createElement('Problem');
			$e_Problems->appendChild($e_Problem);
			$e_CCRDataObjectID = $this->CDAdocument->createElement('CCRDataObjectID', 'PROB' . $pCount);
			$e_Problem->appendChild($e_CCRDataObjectID);
			$e_DateTime = $this->CDAdocument->createElement('DateTime');
			$e_Problem->appendChild($e_DateTime);
			$date = new DateTime($row['begin_date']);
			$e_ExactDateTime = $this->CDAdocument->createElement('ExactDateTime', $date->format('Y-m-d\TH:i:s\Z'));
			$e_DateTime->appendChild($e_ExactDateTime);
			$e_IDs = $this->CDAdocument->createElement('IDs');
			$e_Problem->appendChild($e_IDs);
			$e_ID = $this->CDAdocument->createElement('ID', $row['pid']);
			$e_IDs->appendChild($e_ID);
			$e_IDs->appendChild($this->sourceType($this->sourceID));
			$e_Type = $this->CDAdocument->createElement('Type');
			$e_Problem->appendChild($e_Type);
			$e_Text = $this->CDAdocument->createElement('Text', 'Problem'); // Changed to pass through validator, Problem type must be one of the required string values: Problem, Condition, Diagnosis, Symptom, Finding, Complaint, Functional Limitation.
			$e_Type->appendChild($e_Text);
			$e_Description = $this->CDAdocument->createElement('Description');
			$e_Problem->appendChild($e_Description);
			$e_Text = $this->CDAdocument->createElement('Text', 'lookup_code_descriptions');
			$e_Description->appendChild($e_Text);
			$e_Code = $this->CDAdocument->createElement('Code');
			$e_Description->appendChild($e_Code);
			$e_Value = $this->CDAdocument->createElement('Value', $row['code']);
			$e_Code->appendChild($e_Value);
			$e_Value = $this->CDAdocument->createElement('CodingSystem', 'ICD9-CM');
			$e_Code->appendChild($e_Value);
			$e_Status = $this->CDAdocument->createElement('Status');
			$e_Problem->appendChild($e_Status);
			$e_Text = $this->CDAdocument->createElement('Text', 'Active');
			...
			$e_CommentID = $this->CDAdocument->createElement('CommentID', $row['outcome']);
			$e_Problem->appendChild($e_CommentID);
			$e_Episodes = $this->CDAdocument->createElement('Episodes');
			$e_Problem->appendChild($e_Episodes);
			$e_Number = $this->CDAdocument->createElement('Number');
			$e_Episodes->appendChild($e_Number);
			$e_Episode = $this->CDAdocument->createElement('Episode');
			$e_Episodes->appendChild($e_Episode);
			$e_CCRDataObjectID = $this->CDAdocument->createElement('CCRDataObjectID', 'EP' . $pCount);
			...
			$e_HealthStatus = $this->CDAdocument->createElement('HealthStatus');
			$e_Problem->appendChild($e_HealthStatus);
			$e_DateTime = $this->CDAdocument->createElement('DateTime');
			$e_HealthStatus->appendChild($e_DateTime);
			$e_ExactDateTime = $this->CDAdocument->createElement('ExactDateTime');
			$e_DateTime->appendChild($e_ExactDateTime);
			$e_Description = $this->CDAdocument->createElement('Description');
			$e_HealthStatus->appendChild($e_Description);
			$e_Text = $this->CDAdocument->createElement('Text', $row['code_text']);
			$e_Description->appendChild($e_Text);
	...
	function createAlerts($e_Alerts)
	{
		...
		foreach($data AS $row)
        {
			...
			$e_Alert = $this->CDAdocument->createElement('Alert');
			$e_Alerts->appendChild($e_Alert);
			$e_CCRDataObjectID = $this->CDAdocument->createElement('CCRDataObjectID', UUID::v4());
			...
			$e_Text = $this->CDAdocument->createElement('Text', $row['type'] . '-' . $row['alert_title']);
			$e_Type->appendChild($e_Text);
			$e_Description = $this->CDAdocument->createElement('Description');
			...
			$e_Code = $this->CDAdocument->createElement('Code');
			$e_Description->appendChild($e_Code);
			$e_Value = $this->CDAdocument->createElement('Value', $row['diagnosis']);
			$e_Code->appendChild($e_Value);
			$e_Alert->appendChild($this->sourceType($this->sourceID));
			$e_Agent = $this->CDAdocument->createElement('Agent');
			$e_Alert->appendChild($e_Agent);
			$e_EnvironmentalAgents = $this->CDAdocument->createElement('EnvironmentalAgents');
			$e_Agent->appendChild($e_EnvironmentalAgents);
			$e_EnvironmentalAgent = $this->CDAdocument->createElement('EnvironmentalAgent');
			...
			$e_ExactDateTime = $this->CDAdocument->createElement('ExactDateTime', $row['date']);
			$e_DateTime->appendChild($e_ExactDateTime);
			$e_Description = $this->CDAdocument->createElement('Description');
			$e_EnvironmentalAgent->appendChild($e_Description);
			$e_Text = $this->CDAdocument->createElement('Text', $row['alert_title']);
			$e_Description->appendChild($e_Text);
			$e_Code = $this->CDAdocument->createElement('Code');
			$e_Description->appendChild($e_Code);
			$e_Value = $this->CDAdocument->createElement('Value'); //,$row['codetext']
			$e_Code->appendChild($e_Value);
			$e_Status = $this->CDAdocument->createElement('Status');
			$e_EnvironmentalAgent->appendChild($e_Status);
			$e_Text = $this->CDAdocument->createElement('Text', $row['outcome']);
			$e_Status->appendChild($e_Text);
			$e_EnvironmentalAgent->appendChild($this->sourceType($this->sourceID));
			$e_Reaction = $this->CDAdocument->createElement('Reaction');
			$e_Alert->appendChild($e_Reaction);
			$e_Description = $this->CDAdocument->createElement('Description');
			$e_Reaction->appendChild($e_Description);
			$e_Text = $this->CDAdocument->createElement('Text', $row['reaction']);
			$e_Description->appendChild($e_Text);
			$e_Status = $this->CDAdocument->createElement('Status');
			$e_Reaction->appendChild($e_Status);
			$e_Text = $this->CDAdocument->createElement('Text', 'None');
			$e_Status->appendChild($e_Text);
		}
	...
	function createMedications($e_Medications)
	{
		...
		foreach($data AS $row)
        {
			$e_Medication = $this->CDAdocument->createElement('Medication');
			...
			$e_Text = $this->CDAdocument->createElement('Text', 'Medication');
			$e_Type->appendChild($e_Text);
			$e_Status = $this->CDAdocument->createElement('Status');
			$e_Medication->appendChild($e_Status);
			$e_Text = $this->CDAdocument->createElement('Text', ($row['end_date'] == null || $row['end_date'] == '' ? 'Active' : 'Prior History No Longer Active' ));
			$e_Status->appendChild($e_Text);
			$e_Medication->appendChild($this->sourceType($this->sourceID));
			$e_Product = $this->CDAdocument->createElement('Product');
			$e_Medication->appendChild($e_Product);
			$e_ProductName = $this->CDAdocument->createElement('ProductName');
			$e_Product->appendChild($e_ProductName);
			$e_Text = $this->CDAdocument->createElement('Text', $row['STR']);
			$e_ProductName->appendChild(clone $e_Text);
			$e_Code = $this->CDAdocument->createElement('Code');
			$e_ProductName->appendChild($e_Code);
			$e_Value = $this->CDAdocument->createElement('Value', $row['CODE']);
			$e_Code->appendChild($e_Value);
			$e_Value = $this->CDAdocument->createElement('CodingSystem', 'RxNorm');
			$e_Code->appendChild($e_Value);
			$e_Strength = $this->CDAdocument->createElement('Strength');
			$e_Product->appendChild($e_Strength);
			$e_Value = $this->CDAdocument->createElement('Value', $row['dose']);
			$e_Strength->appendChild($e_Value);
			$e_Units = $this->CDAdocument->createElement('Units');
			$e_Strength->appendChild($e_Units);
			$e_Unit = $this->CDAdocument->createElement('Unit',' - ');
			$e_Units->appendChild($e_Unit);
			$e_Form = $this->CDAdocument->createElement('Form');
			$e_Product->appendChild($e_Form);
			$e_Text = $this->CDAdocument->createElement('Text', $row['form']);
			$e_Form->appendChild($e_Text);
			$e_Quantity = $this->CDAdocument->createElement('Quantity');
			$e_Medication->appendChild($e_Quantity);
			$e_Value = $this->CDAdocument->createElement('Value', $row['dispense']);
			$e_Quantity->appendChild($e_Value);
			$e_Units = $this->CDAdocument->createElement('Units');
			$e_Quantity->appendChild($e_Units);
			$e_Unit = $this->CDAdocument->createElement('Unit', ' - ');
			$e_Units->appendChild($e_Unit);
			$e_Directions = $this->CDAdocument->createElement('Directions');
			$e_Medication->appendChild($e_Directions);
			$e_Direction = $this->CDAdocument->createElement('Direction');
			$e_Directions->appendChild($e_Direction);
			$e_Description = $this->CDAdocument->createElement('Description');
			$e_Direction->appendChild($e_Description);
			$e_Text = $this->CDAdocument->createElement('Text', $row['prescription_often']);
			$e_Description->appendChild(clone $e_Text);
			$e_Route = $this->CDAdocument->createElement('Route');
			$e_Direction->appendChild($e_Route);
			$e_Text = $this->CDAdocument->createElement('Text', $row['route']);
			$e_Route->appendChild($e_Text);
			$e_Site = $this->CDAdocument->createElement('Site');
			$e_Direction->appendChild($e_Site);
			$e_Text = $this->CDAdocument->createElement('Text', '????');
			$e_Site->appendChild($e_Text);
			$e_PatientInstructions = $this->CDAdocument->createElement('PatientInstructions');
			$e_Medication->appendChild($e_PatientInstructions);
			$e_Instruction = $this->CDAdocument->createElement('Instruction');
			$e_PatientInstructions->appendChild($e_Instruction);
			$e_Text = $this->CDAdocument->createElement('Text', $row['prescription_often']);
			$e_Instruction->appendChild($e_Text);
			$e_Refills = $this->CDAdocument->createElement('Refills');
			$e_Medication->appendChild($e_Refills);
			$e_Refill = $this->CDAdocument->createElement('Refill');
			$e_Refills->appendChild($e_Refill);
			$e_Number = $this->CDAdocument->createElement('Number', $row['refill']);
	...
	function createImmunizations($e_Immunizations)
	{
		...
		foreach($data AS $row)
        {
			$e_Immunization = $this->CDAdocument->createElement('Immunization');
			...
			$e_Text = $this->CDAdocument->createElement('Text', 'Immunization');
			$e_Type->appendChild($e_Text);
			...
			$e_ProductName = $this->CDAdocument->createElement('ProductName');
			$e_Product->appendChild($e_ProductName);
			$e_Text = $this->CDAdocument->createElement('Text',$row['vaccine_name']);
			$e_ProductName->appendChild($e_Text);
			$e_Code = $this->CDAdocument->createElement('Code',$row['code']);
			$e_ProductName->appendChild($e_Code);
			...
			$e_Direction->appendChild($e_Description);
			$e_Text = $this->CDAdocument->createElement('Text',$row['note']);
			$e_Description->appendChild($e_Text);
			$e_Code = $this->CDAdocument->createElement('Code');
			$e_Description->appendChild($e_Code);
			$e_Value = $this->CDAdocument->createElement('Value', 'None');
			$e_Code->appendChild($e_Value);
	...
	function createResults($e_Results)
	{
		...
		foreach($data AS $row)
        {
			$e_Result = $this->CDAdocument->createElement('Result');
			...
			$e_ID = $this->CDAdocument->createElement('ID');
			$e_IDs->appendChild($e_ID);
			...
			$e_ActorID = $this->CDAdocument->createElement('ActorID', $this->authorID);
			$e_Actor->appendChild($e_ActorID);
			$e_Test = $this->CDAdocument->createElement('Test');
			...
			$e_Text = $this->CDAdocument->createElement('Text', 'Observation');
			$e_Type->appendChild($e_Text);
			$e_Description = $this->CDAdocument->createElement('Description');
			$e_Test->appendChild($e_Description);
			$e_Text = $this->CDAdocument->createElement('Text', $row['name']);
			$e_Description->appendChild($e_Text);
			$e_Code = $this->CDAdocument->createElement('Code');
			$e_Description->appendChild($e_Code);
			$e_Value = $this->CDAdocument->createElement('Value', 'Value');
			$e_Code->appendChild($e_Value);
			$e_Source = $this->CDAdocument->createElement('Source');
			...
			$e_ActorID = $this->CDAdocument->createElement('ActorID', $this->authorID);
			$e_Actor->appendChild($e_ActorID);
			$e_TestResult = $this->CDAdocument->createElement('TestResult');
			$e_Test->appendChild($e_TestResult);
			$e_Value = $this->CDAdocument->createElement('Value', $row['result']);
			...
			$e_Text = $this->CDAdocument->createElement('Text', $row['result']);
			$e_Description->appendChild($e_Text);
			$e_NormalResult = $this->CDAdocument->createElement('NormalResult');
			$e_Test->appendChild($e_NormalResult);
			$e_Normal = $this->CDAdocument->createElement('Normal');
			$e_NormalResult->appendChild($e_Normal);
			$e_Value = $this->CDAdocument->createElement('Value', $row['range']);
			$e_Normal->appendChild($e_Value);
			$e_Units = $this->CDAdocument->createElement('Units');
			$e_Normal->appendChild($e_Units);
			$e_Unit = $this->CDAdocument->createElement('Unit', 'Test Unit');
			$e_Units->appendChild($e_Unit);
			$e_Source = $this->CDAdocument->createElement('Source');
			...
			$e_ActorID = $this->CDAdocument->createElement('ActorID', $this->authorID);
			$e_Actor->appendChild($e_ActorID);
			$e_Flag = $this->CDAdocument->createElement('Flag');
			$e_Test->appendChild($e_Flag);
			$e_Text = $this->CDAdocument->createElement('Text', $row['abnormal']);
	...
	function createActors($e_Actors)
	{
        ...
        $e_ActorObjectID = $this->CDAdocument->createElement('ActorObjectID', 'A1234'); // Refer createCCRHeader.php
        $e_Actor->appendChild($e_ActorObjectID);
        $e_Person = $this->CDAdocument->createElement('Person');
        $e_Actor->appendChild($e_Person);
        $e_Name = $this->CDAdocument->createElement('Name');
        $e_Person->appendChild($e_Name);
        $e_CurrentName = $this->CDAdocument->createElement('CurrentName');
        $e_Name->appendChild($e_CurrentName);
        $e_Given = $this->CDAdocument->createElement('Given', $p['fname']);
        $e_CurrentName->appendChild($e_Given);
        $e_Family = $this->CDAdocument->createElement('Family', $p['lname']);
        $e_CurrentName->appendChild($e_Family);
        $e_Suffix = $this->CDAdocument->createElement('Suffix');
        ...
        $e_Text = $this->CDAdocument->createElement('Text', $p['sex']);
        $e_Gender->appendChild($e_Text);
        $e_Code = $this->CDAdocument->createElement('Code');
        $e_Gender->appendChild($e_Code);
        $e_Value = $this->CDAdocument->createElement('Value');
        $e_Code->appendChild($e_Value);
        $e_IDs = $this->CDAdocument->createElement('IDs');
        $e_Actor->appendChild($e_IDs);
        $e_Type = $this->CDAdocument->createElement('Type');
        $e_IDs->appendChild($e_Type);
        $e_Text = $this->CDAdocument->createElement('Text', 'Patient ID');
        $e_Type->appendChild($e_Text);
        $e_ID = $this->CDAdocument->createElement('ID', $p['pid']);
        $e_IDs->appendChild($e_ID);
        $e_Source = $this->CDAdocument->createElement('Source');
        $e_IDs->appendChild($e_Source);
        $e_SourceActor = $this->CDAdocument->createElement('Actor');
        $e_Source->appendChild($e_SourceActor);
        $e_ActorID = $this->CDAdocument->createElement('ActorID', UUID::v4());
        $e_SourceActor->appendChild($e_ActorID);
        // address
        $e_Address = $this->CDAdocument->createElement('Address');
        $e_Actor->appendChild($e_Address);
        $e_Type = $this->CDAdocument->createElement('Type');
        $e_Address->appendChild($e_Type);
        $e_Text = $this->CDAdocument->createElement('Text', 'H');
        $e_Type->appendChild($e_Text);
        $e_Line1 = $this->CDAdocument->createElement('Line1', $p['address']);
        $e_Address->appendChild($e_Line1);
        $e_Line2 = $this->CDAdocument->createElement('Line2');
        $e_Address->appendChild($e_Line1);
        $e_City = $this->CDAdocument->createElement('City', $p['city']);
        $e_Address->appendChild($e_City);
        $e_State = $this->CDAdocument->createElement('State', $p['state']);
        $e_Address->appendChild($e_State);
        $e_PostalCode = $this->CDAdocument->createElement('PostalCode', $p['zipcode']);
        $e_Address->appendChild($e_PostalCode);
        $e_Telephone = $this->CDAdocument->createElement('Telephone');
        $e_Actor->appendChild($e_Telephone);
        $e_Value = $this->CDAdocument->createElement('Value', $p['home_phone']);
		...
		$e_Actor = $this->CDAdocument->createElement('Actor');
		$e_Actors->appendChild($e_Actor);
		$e_ActorObjectID = $this->CDAdocument->createElement('ActorObjectID', $this->authorID);
		$e_Actor->appendChild($e_ActorObjectID);
		$e_InformationSystem = $this->CDAdocument->createElement('InformationSystem');
		$e_Actor->appendChild($e_InformationSystem);
		$e_Name = $this->CDAdocument->createElement('Name', $informationData['facility']);
		$e_InformationSystem->appendChild($e_Name);
		$e_Type = $this->CDAdocument->createElement('Type', 'Facility');
		...
		$e_Type = $this->CDAdocument->createElement('Type');
		$e_Address->appendChild($e_Type);
		$e_Text = $this->CDAdocument->createElement('Text', 'WP');
		$e_Type->appendChild($e_Text);
		$e_Line1 = $this->CDAdocument->createElement('Line1', $informationData['street']);
		$e_Address->appendChild($e_Line1);
		$e_Line2 = $this->CDAdocument->createElement('Line2');
		$e_Address->appendChild($e_Line1);
		$e_City = $this->CDAdocument->createElement('City', $informationData['city']);
		$e_Address->appendChild($e_City);
		$e_State = $this->CDAdocument->createElement('State', $informationData['state'] . ' ');
		$e_Address->appendChild($e_State);
		$e_PostalCode = $this->CDAdocument->createElement('PostalCode', $informationData['postal_code']);
		$e_Address->appendChild($e_PostalCode);
		$e_Telephone = $this->CDAdocument->createElement('Telephone');
		$e_Actor->appendChild($e_Telephone);
		$e_Phone = $this->CDAdocument->createElement('Value', $informationData['phone']);
		...
		$e_Actor = $this->CDAdocument->createElement('Actor');
		$e_Actors->appendChild($e_Actor);
		$e_ActorObjectID = $this->CDAdocument->createElement('ActorObjectID', $this->gaiaID);
		$e_Actor->appendChild($e_ActorObjectID);
		$e_InformationSystem = $this->CDAdocument->createElement('InformationSystem');
		$e_Actor->appendChild($e_InformationSystem);
		$e_Name = $this->CDAdocument->createElement('Name', 'GEHR');
		$e_InformationSystem->appendChild($e_Name);
		$e_Type = $this->CDAdocument->createElement('Type', 'GaiaEHR');
		$e_InformationSystem->appendChild($e_Type);
		$e_Version = $this->CDAdocument->createElement('Version', '1.x');
		$e_InformationSystem->appendChild($e_Version);
		$e_IDs = $this->CDAdocument->createElement('IDs');
		$e_Actor->appendChild($e_IDs);
		$e_Type = $this->CDAdocument->createElement('Type');
		$e_IDs->appendChild($e_Type);
		$e_Text = $this->CDAdocument->createElement('Text', 'Certification #');
		$e_Type->appendChild($e_Text);
		$e_ID = $this->CDAdocument->createElement('ID', 'NONE');
		$e_IDs->appendChild($e_ID);
		$e_Source = $this->CDAdocument->createElement('Source');
		$e_IDs->appendChild($e_Source);
		$e_SourceActor = $this->CDAdocument->createElement('Actor');
		...
		$e_Text = $this->CDAdocument->createElement('Text', 'WP');
		$e_Type->appendChild($e_Text);
		$e_Line1 = $this->CDAdocument->createElement('Line1', '90 Blvd. Media Luna');
		$e_Address->appendChild($e_Line1);
		$e_Line2 = $this->CDAdocument->createElement('Line2');
		$e_Address->appendChild($e_Line1);
		$e_City = $this->CDAdocument->createElement('City', 'San Juan');
		$e_Address->appendChild($e_City);
		$e_State = $this->CDAdocument->createElement('State', 'PR ');
		$e_Address->appendChild($e_State);
		$e_PostalCode = $this->CDAdocument->createElement('PostalCode', '00987');
		$e_Address->appendChild($e_PostalCode);
		$e_Telephone = $this->CDAdocument->createElement('Telephone');
		$e_Actor->appendChild($e_Telephone);
		$e_Phone = $this->CDAdocument->createElement('Value', '000-000-0000');
		$e_Telephone->appendChild($e_Phone);
		$e_Source = $this->CDAdocument->createElement('Source');
		$e_Actor->appendChild($e_Source);
		$e_Actor = $this->CDAdocument->createElement('Actor');
		...
		foreach($labData AS $row)
        {
			$e_Actor = $this->CDAdocument->createElement('Actor');
			$e_Actors->appendChild($e_Actor);
			$e_ActorObjectID = $this->CDAdocument->createElement('ActorObjectID', ${"labID{$row['id']}"});
			$e_Actor->appendChild($e_ActorObjectID);
			$e_InformationSystem = $this->CDAdocument->createElement('InformationSystem');
			$e_Actor->appendChild($e_InformationSystem);
			$e_Name = $this->CDAdocument->createElement('Name', $row['lname'] . " " . $row['fname']);
			$e_InformationSystem->appendChild($e_Name);
			$e_Type = $this->CDAdocument->createElement('Type', 'Lab Service');
			$e_InformationSystem->appendChild($e_Type);
			$e_IDs = $this->CDAdocument->createElement('IDs');
			$e_Actor->appendChild($e_IDs);
			$e_Type = $this->CDAdocument->createElement('Type');
			...
			$e_Text = $this->CDAdocument->createElement('Text', 'WP');
			$e_Type->appendChild($e_Text);
			$e_Line1 = $this->CDAdocument->createElement('Line1', $row['street']);
			$e_Address->appendChild($e_Line1);
			$e_Line2 = $this->CDAdocument->createElement('Line2');
			$e_Address->appendChild($e_Line1);
			$e_City = $this->CDAdocument->createElement('City', $row['city']);
			$e_Address->appendChild($e_City);
			$e_State = $this->CDAdocument->createElement('State', $row['state'] . ' ');
			$e_Address->appendChild($e_State);
			$e_PostalCode = $this->CDAdocument->createElement('PostalCode', $row['zip']);
			$e_Address->appendChild($e_PostalCode);
			$e_Telephone = $this->CDAdocument->createElement('Telephone');
			$e_Actor->appendChild($e_Telephone);
			$e_Phone = $this->CDAdocument->createElement('Value', $row['phone']);
			$e_Telephone->appendChild($e_Phone);
			$e_Source = $this->CDAdocument->createElement('Source');
			$e_Actor->appendChild($e_Source);
			$e_Actor = $this->CDAdocument->createElement('Actor');
