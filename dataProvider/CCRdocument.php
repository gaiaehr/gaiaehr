...
class CCR
{
	...
	{
		$this->CCRdocument = new DOMDocument('1.0', 'UTF-8');
    ...
    function createCCR($request)
	{
		...
		$e_styleSheet = $this->CCRdocument->createProcessingInstruction('xml-stylesheet', 'type="text/xsl" href="'.$_SESSION['url'].'/lib/ccr/stylesheet/ccr.xsl"');
		$this->CCRdocument->appendChild($e_styleSheet);
		 ...
		 * Problems
		 */
		$e_Problems = $this->CCRdocument->createElement('Problems');
		$this->createProblem($e_Problems);
		$e_Body->appendChild($e_Problems);
		/**
		 * Alerts
		 */
		$e_Alerts = $this->CCRdocument->createElement('Alerts');
		$this->createAlerts($e_Alerts);
		$e_Body->appendChild($e_Alerts);
		/**
		 * Medication
		 */
		$e_Medications = $this->CCRdocument->createElement('Medications');
		$this->createMedications($e_Medications);
		$e_Body->appendChild($e_Medications);
		/**
		 * Immunization
		 */
		$e_Immunizations = $this->CCRdocument->createElement('Immunizations');
		$this->createImmunizations($e_Immunizations);
		$e_Body->appendChild($e_Immunizations);
		/**
		 * Results
		 */
		$e_Results = $this->CCRdocument->createElement('Results');
		 ...
		 * Actors
		 */
		$e_Actors = $this->CCRdocument->createElement('Actors');
	...
	function generateCCR($raw)
	{
        ...
        else
        {
			...
			print $this->CCRdocument->saveXml();
    	}
	}

	function sourceType($uuid)
	{
		$e_Source = $this->CCRdocument->createElement('Source');
		$e_Actor  = $this->CCRdocument->createElement('Actor');
		$e_Source->appendChild($e_Actor);
		$e_ActorID = $this->CCRdocument->createElement('ActorID', $uuid);
	...
	{
		// save the raw xml
		$main_xml = $this->CCRdocument->saveXml();
		// save the stylesheet
	...
	{
		$e_ccrDocObjID = $this->CCRdocument->createElement('CCRDocumentObjectID', UUID::v4());
		$e_ccr->appendChild($e_ccrDocObjID);
		$e_Language = $this->CCRdocument->createElement('Language');
		$e_ccr->appendChild($e_Language);
		$e_Text = $this->CCRdocument->createElement('Text', 'English');
		$e_Language->appendChild($e_Text);
		$e_Version = $this->CCRdocument->createElement('Version', 'V1.0');
		$e_ccr->appendChild($e_Version);
		$e_dateTime = $this->CCRdocument->createElement('DateTime');
		$e_ccr->appendChild($e_dateTime);
		$e_ExactDateTime = $this->CCRdocument->createElement('ExactDateTime', date('Y-m-d\TH:i:s\Z'));
		$e_dateTime->appendChild($e_ExactDateTime);
		$e_patient = $this->CCRdocument->createElement('Patient');
		$e_ccr->appendChild($e_patient);
		$e_ActorID = $this->CCRdocument->createElement('ActorID', 'A1234'); // This value and ActorID in createCCRActor.php should be same.
		$e_patient->appendChild($e_ActorID);

		//Header From:
		$e_From = $this->CCRdocument->createElement('From');
		$e_ccr->appendChild($e_From);
		$e_ActorLink = $this->CCRdocument->createElement('ActorLink');
		$e_From->appendChild($e_ActorLink);
		$e_ActorID = $this->CCRdocument->createElement('ActorID', $this->authorID);
		$e_ActorLink->appendChild($e_ActorID);
		$e_ActorRole = $this->CCRdocument->createElement('ActorRole');
		$e_ActorLink->appendChild($e_ActorRole);
		$e_Text = $this->CCRdocument->createElement('Text', 'author');
		$e_ActorRole->appendChild($e_Text);

		//Header To:
		$e_To = $this->CCRdocument->createElement('To');
		$e_ccr->appendChild($e_To);
		$e_ActorLink = $this->CCRdocument->createElement('ActorLink');
		$e_To->appendChild($e_ActorLink);
		$e_ActorID = $this->CCRdocument->createElement('ActorID', 'A1234');
		$e_ActorLink->appendChild($e_ActorID);
		$e_ActorRole = $this->CCRdocument->createElement('ActorRole');
		$e_ActorLink->appendChild($e_ActorRole);
		$e_Text = $this->CCRdocument->createElement('Text', 'patient');
		$e_ActorRole->appendChild($e_Text);

		//Header Purpose:
		$e_Purpose = $this->CCRdocument->createElement('Purpose');
		$e_ccr->appendChild($e_Purpose);
		$e_Description = $this->CCRdocument->createElement('Description');
		$e_Purpose->appendChild($e_Description);
		$e_Text = $this->CCRdocument->createElement('Text', 'Summary of patient information');
	...
	function createProblem($e_Problems)
	{
        ...
        {
			$pCount++;
			$e_Problem = $this->CCRdocument->createElement('Problem');
			$e_Problems->appendChild($e_Problem);
			$e_CCRDataObjectID = $this->CCRdocument->createElement('CCRDataObjectID', 'PROB' . $pCount);
			...
			$e_ID = $this->CCRdocument->createElement('ID', $row['pid']);
			$e_IDs->appendChild($e_ID);
			$e_IDs->appendChild($this->sourceType($this->sourceID));
			$e_Type = $this->CCRdocument->createElement('Type');
			$e_Problem->appendChild($e_Type);
			$e_Text = $this->CCRdocument->createElement('Text', 'Problem'); // Changed to pass through validator, Problem type must be one of the required string values: Problem, Condition, Diagnosis, Symptom, Finding, Complaint, Functional Limitation.
			$e_Type->appendChild($e_Text);
			$e_Description = $this->CCRdocument->createElement('Description');
			$e_Problem->appendChild($e_Description);
			$e_Text = $this->CCRdocument->createElement('Text', 'lookup_code_descriptions');
			$e_Description->appendChild($e_Text);
			$e_Code = $this->CCRdocument->createElement('Code');
			$e_Description->appendChild($e_Code);
			$e_Value = $this->CCRdocument->createElement('Value', $row['code']);
			$e_Code->appendChild($e_Value);
			$e_Value = $this->CCRdocument->createElement('CodingSystem', 'ICD9-CM');
			$e_Code->appendChild($e_Value);
			$e_Status = $this->CCRdocument->createElement('Status');
			$e_Problem->appendChild($e_Status);
			$e_Text = $this->CCRdocument->createElement('Text', 'Active');
			...
			$e_CommentID = $this->CCRdocument->createElement('CommentID', $row['outcome']);
			$e_Problem->appendChild($e_CommentID);
			$e_Episodes = $this->CCRdocument->createElement('Episodes');
			$e_Problem->appendChild($e_Episodes);
			$e_Number = $this->CCRdocument->createElement('Number');
			$e_Episodes->appendChild($e_Number);
			$e_Episode = $this->CCRdocument->createElement('Episode');
			$e_Episodes->appendChild($e_Episode);
			$e_CCRDataObjectID = $this->CCRdocument->createElement('CCRDataObjectID', 'EP' . $pCount);
			...
			$e_HealthStatus = $this->CCRdocument->createElement('HealthStatus');
			$e_Problem->appendChild($e_HealthStatus);
			$e_DateTime = $this->CCRdocument->createElement('DateTime');
			$e_HealthStatus->appendChild($e_DateTime);
			$e_ExactDateTime = $this->CCRdocument->createElement('ExactDateTime');
			$e_DateTime->appendChild($e_ExactDateTime);
			$e_Description = $this->CCRdocument->createElement('Description');
			$e_HealthStatus->appendChild($e_Description);
			$e_Text = $this->CCRdocument->createElement('Text', $row['code_text']);
			$e_Description->appendChild($e_Text);
	...
	function createAlerts($e_Alerts)
	{
		...
		foreach($data AS $row)
        {
			...
			$e_Alert = $this->CCRdocument->createElement('Alert');
			$e_Alerts->appendChild($e_Alert);
			$e_CCRDataObjectID = $this->CCRdocument->createElement('CCRDataObjectID', UUID::v4());
			...
			$e_Text = $this->CCRdocument->createElement('Text', $row['type'] . '-' . $row['alert_title']);
			$e_Type->appendChild($e_Text);
			$e_Description = $this->CCRdocument->createElement('Description');
			...
			$e_Code = $this->CCRdocument->createElement('Code');
			$e_Description->appendChild($e_Code);
			$e_Value = $this->CCRdocument->createElement('Value', $row['diagnosis']);
			$e_Code->appendChild($e_Value);
			$e_Alert->appendChild($this->sourceType($this->sourceID));
			$e_Agent = $this->CCRdocument->createElement('Agent');
			$e_Alert->appendChild($e_Agent);
			$e_EnvironmentalAgents = $this->CCRdocument->createElement('EnvironmentalAgents');
			$e_Agent->appendChild($e_EnvironmentalAgents);
			$e_EnvironmentalAgent = $this->CCRdocument->createElement('EnvironmentalAgent');
			...
			$e_ExactDateTime = $this->CCRdocument->createElement('ExactDateTime', $row['date']);
			$e_DateTime->appendChild($e_ExactDateTime);
			$e_Description = $this->CCRdocument->createElement('Description');
			$e_EnvironmentalAgent->appendChild($e_Description);
			$e_Text = $this->CCRdocument->createElement('Text', $row['alert_title']);
			$e_Description->appendChild($e_Text);
			$e_Code = $this->CCRdocument->createElement('Code');
			$e_Description->appendChild($e_Code);
			$e_Value = $this->CCRdocument->createElement('Value'); //,$row['codetext']
			$e_Code->appendChild($e_Value);
			$e_Status = $this->CCRdocument->createElement('Status');
			$e_EnvironmentalAgent->appendChild($e_Status);
			$e_Text = $this->CCRdocument->createElement('Text', $row['outcome']);
			$e_Status->appendChild($e_Text);
			$e_EnvironmentalAgent->appendChild($this->sourceType($this->sourceID));
			$e_Reaction = $this->CCRdocument->createElement('Reaction');
			$e_Alert->appendChild($e_Reaction);
			$e_Description = $this->CCRdocument->createElement('Description');
			$e_Reaction->appendChild($e_Description);
			$e_Text = $this->CCRdocument->createElement('Text', $row['reaction']);
			$e_Description->appendChild($e_Text);
			$e_Status = $this->CCRdocument->createElement('Status');
			$e_Reaction->appendChild($e_Status);
			$e_Text = $this->CCRdocument->createElement('Text', 'None');
			$e_Status->appendChild($e_Text);
		}
	...
	function createMedications($e_Medications)
	{
		...
		foreach($data AS $row)
        {
			$e_Medication = $this->CCRdocument->createElement('Medication');
			...
			$e_Text = $this->CCRdocument->createElement('Text', 'Medication');
			$e_Type->appendChild($e_Text);
			$e_Status = $this->CCRdocument->createElement('Status');
			$e_Medication->appendChild($e_Status);
			$e_Text = $this->CCRdocument->createElement('Text', ($row['end_date'] == null || $row['end_date'] == '' ? 'Active' : 'Prior History No Longer Active' ));
			$e_Status->appendChild($e_Text);
			$e_Medication->appendChild($this->sourceType($this->sourceID));
			$e_Product = $this->CCRdocument->createElement('Product');
			$e_Medication->appendChild($e_Product);
			$e_ProductName = $this->CCRdocument->createElement('ProductName');
			$e_Product->appendChild($e_ProductName);
			$e_Text = $this->CCRdocument->createElement('Text', $row['STR']);
			$e_ProductName->appendChild(clone $e_Text);
			$e_Code = $this->CCRdocument->createElement('Code');
			$e_ProductName->appendChild($e_Code);
			$e_Value = $this->CCRdocument->createElement('Value', $row['CODE']);
			$e_Code->appendChild($e_Value);
			$e_Value = $this->CCRdocument->createElement('CodingSystem', 'RxNorm');
			$e_Code->appendChild($e_Value);
			$e_Strength = $this->CCRdocument->createElement('Strength');
			$e_Product->appendChild($e_Strength);
			$e_Value = $this->CCRdocument->createElement('Value', $row['dose']);
			$e_Strength->appendChild($e_Value);
			$e_Units = $this->CCRdocument->createElement('Units');
			$e_Strength->appendChild($e_Units);
			$e_Unit = $this->CCRdocument->createElement('Unit',' - ');
			$e_Units->appendChild($e_Unit);
			$e_Form = $this->CCRdocument->createElement('Form');
			$e_Product->appendChild($e_Form);
			$e_Text = $this->CCRdocument->createElement('Text', $row['form']);
			$e_Form->appendChild($e_Text);
			$e_Quantity = $this->CCRdocument->createElement('Quantity');
			$e_Medication->appendChild($e_Quantity);
			$e_Value = $this->CCRdocument->createElement('Value', $row['dispense']);
			$e_Quantity->appendChild($e_Value);
			$e_Units = $this->CCRdocument->createElement('Units');
			$e_Quantity->appendChild($e_Units);
			$e_Unit = $this->CCRdocument->createElement('Unit', ' - ');
			$e_Units->appendChild($e_Unit);
			$e_Directions = $this->CCRdocument->createElement('Directions');
			$e_Medication->appendChild($e_Directions);
			$e_Direction = $this->CCRdocument->createElement('Direction');
			$e_Directions->appendChild($e_Direction);
			$e_Description = $this->CCRdocument->createElement('Description');
			$e_Direction->appendChild($e_Description);
			$e_Text = $this->CCRdocument->createElement('Text', $row['prescription_often']);
			$e_Description->appendChild(clone $e_Text);
			$e_Route = $this->CCRdocument->createElement('Route');
			$e_Direction->appendChild($e_Route);
			$e_Text = $this->CCRdocument->createElement('Text', $row['route']);
			$e_Route->appendChild($e_Text);
			$e_Site = $this->CCRdocument->createElement('Site');
			$e_Direction->appendChild($e_Site);
			$e_Text = $this->CCRdocument->createElement('Text', '????');
			$e_Site->appendChild($e_Text);
			$e_PatientInstructions = $this->CCRdocument->createElement('PatientInstructions');
			$e_Medication->appendChild($e_PatientInstructions);
			$e_Instruction = $this->CCRdocument->createElement('Instruction');
			$e_PatientInstructions->appendChild($e_Instruction);
			$e_Text = $this->CCRdocument->createElement('Text', $row['prescription_often']);
			$e_Instruction->appendChild($e_Text);
			$e_Refills = $this->CCRdocument->createElement('Refills');
			$e_Medication->appendChild($e_Refills);
			$e_Refill = $this->CCRdocument->createElement('Refill');
			$e_Refills->appendChild($e_Refill);
			$e_Number = $this->CCRdocument->createElement('Number', $row['refill']);
    ...
    function createImmunizations($e_Immunizations)
	{
		...
		foreach($data AS $row)
        {
			$e_Immunization = $this->CCRdocument->createElement('Immunization');
			...
			$e_Text = $this->CCRdocument->createElement('Text', 'Immunization');
			$e_Type->appendChild($e_Text);
			...
			$e_ProductName = $this->CCRdocument->createElement('ProductName');
			$e_Product->appendChild($e_ProductName);
			$e_Text = $this->CCRdocument->createElement('Text',$row['vaccine_name']);
			$e_ProductName->appendChild($e_Text);
			$e_Code = $this->CCRdocument->createElement('Code',$row['code']);
			$e_ProductName->appendChild($e_Code);
			...
			$e_Direction->appendChild($e_Description);
			$e_Text = $this->CCRdocument->createElement('Text',$row['note']);
			$e_Description->appendChild($e_Text);
			$e_Code = $this->CCRdocument->createElement('Code');
			$e_Description->appendChild($e_Code);
			$e_Value = $this->CCRdocument->createElement('Value', 'None');
			$e_Code->appendChild($e_Value);
    ...
    function createResults($e_Results)
	{
		...
		foreach($data AS $row)
        {
			$e_Result = $this->CCRdocument->createElement('Result');
			...
			$e_ID = $this->CCRdocument->createElement('ID');
			$e_IDs->appendChild($e_ID);
			...
			$e_ActorID = $this->CCRdocument->createElement('ActorID', $this->authorID);
			$e_Actor->appendChild($e_ActorID);
			$e_Test = $this->CCRdocument->createElement('Test');
			...
			$e_Text = $this->CCRdocument->createElement('Text', 'Observation');
			$e_Type->appendChild($e_Text);
			$e_Description = $this->CCRdocument->createElement('Description');
			$e_Test->appendChild($e_Description);
			$e_Text = $this->CCRdocument->createElement('Text', $row['name']);
			$e_Description->appendChild($e_Text);
			$e_Code = $this->CCRdocument->createElement('Code');
			$e_Description->appendChild($e_Code);
			$e_Value = $this->CCRdocument->createElement('Value', 'Value');
			$e_Code->appendChild($e_Value);
			$e_Source = $this->CCRdocument->createElement('Source');
			...
			$e_ActorID = $this->CCRdocument->createElement('ActorID', $this->authorID);
			$e_Actor->appendChild($e_ActorID);
			$e_TestResult = $this->CCRdocument->createElement('TestResult');
			$e_Test->appendChild($e_TestResult);
			$e_Value = $this->CCRdocument->createElement('Value', $row['result']);
			...
			$e_Text = $this->CCRdocument->createElement('Text', $row['result']);
			$e_Description->appendChild($e_Text);
			$e_NormalResult = $this->CCRdocument->createElement('NormalResult');
			$e_Test->appendChild($e_NormalResult);
			$e_Normal = $this->CCRdocument->createElement('Normal');
			$e_NormalResult->appendChild($e_Normal);
			$e_Value = $this->CCRdocument->createElement('Value', $row['range']);
			$e_Normal->appendChild($e_Value);
			$e_Units = $this->CCRdocument->createElement('Units');
			$e_Normal->appendChild($e_Units);
			$e_Unit = $this->CCRdocument->createElement('Unit', 'Test Unit');
			$e_Units->appendChild($e_Unit);
			$e_Source = $this->CCRdocument->createElement('Source');
			...
			$e_ActorID = $this->CCRdocument->createElement('ActorID', $this->authorID);
			$e_Actor->appendChild($e_ActorID);
			$e_Flag = $this->CCRdocument->createElement('Flag');
			$e_Test->appendChild($e_Flag);
			$e_Text = $this->CCRdocument->createElement('Text', $row['abnormal']);
    ...
    function createActors($e_Actors)
	{
        ...
        $e_ActorObjectID = $this->CCRdocument->createElement('ActorObjectID', 'A1234'); // Refer createCCRHeader.php
        $e_Actor->appendChild($e_ActorObjectID);
        $e_Person = $this->CCRdocument->createElement('Person');
        $e_Actor->appendChild($e_Person);
        $e_Name = $this->CCRdocument->createElement('Name');
        $e_Person->appendChild($e_Name);
        $e_CurrentName = $this->CCRdocument->createElement('CurrentName');
        $e_Name->appendChild($e_CurrentName);
        $e_Given = $this->CCRdocument->createElement('Given', $p['fname']);
        $e_CurrentName->appendChild($e_Given);
        $e_Family = $this->CCRdocument->createElement('Family', $p['lname']);
        $e_CurrentName->appendChild($e_Family);
        $e_Suffix = $this->CCRdocument->createElement('Suffix');
        ...
        $e_Text = $this->CCRdocument->createElement('Text', $p['sex']);
        $e_Gender->appendChild($e_Text);
        $e_Code = $this->CCRdocument->createElement('Code');
        $e_Gender->appendChild($e_Code);
        $e_Value = $this->CCRdocument->createElement('Value');
        $e_Code->appendChild($e_Value);
        $e_IDs = $this->CCRdocument->createElement('IDs');
        $e_Actor->appendChild($e_IDs);
        $e_Type = $this->CCRdocument->createElement('Type');
        $e_IDs->appendChild($e_Type);
        $e_Text = $this->CCRdocument->createElement('Text', 'Patient ID');
        $e_Type->appendChild($e_Text);
        $e_ID = $this->CCRdocument->createElement('ID', $p['pid']);
        $e_IDs->appendChild($e_ID);
        $e_Source = $this->CCRdocument->createElement('Source');
        $e_IDs->appendChild($e_Source);
        $e_SourceActor = $this->CCRdocument->createElement('Actor');
        $e_Source->appendChild($e_SourceActor);
        $e_ActorID = $this->CCRdocument->createElement('ActorID', UUID::v4());
        $e_SourceActor->appendChild($e_ActorID);
        // address
        $e_Address = $this->CCRdocument->createElement('Address');
        $e_Actor->appendChild($e_Address);
        $e_Type = $this->CCRdocument->createElement('Type');
        $e_Address->appendChild($e_Type);
        $e_Text = $this->CCRdocument->createElement('Text', 'H');
        $e_Type->appendChild($e_Text);
        $e_Line1 = $this->CCRdocument->createElement('Line1', $p['address']);
        $e_Address->appendChild($e_Line1);
        $e_Line2 = $this->CCRdocument->createElement('Line2');
        $e_Address->appendChild($e_Line1);
        $e_City = $this->CCRdocument->createElement('City', $p['city']);
        $e_Address->appendChild($e_City);
        $e_State = $this->CCRdocument->createElement('State', $p['state']);
        $e_Address->appendChild($e_State);
        $e_PostalCode = $this->CCRdocument->createElement('PostalCode', $p['zipcode']);
        $e_Address->appendChild($e_PostalCode);
        $e_Telephone = $this->CCRdocument->createElement('Telephone');
        $e_Actor->appendChild($e_Telephone);
        $e_Value = $this->CCRdocument->createElement('Value', $p['home_phone']);
		...
		$e_Actor = $this->CCRdocument->createElement('Actor');
		$e_Actors->appendChild($e_Actor);
		$e_ActorObjectID = $this->CCRdocument->createElement('ActorObjectID', $this->authorID);
		$e_Actor->appendChild($e_ActorObjectID);
		$e_InformationSystem = $this->CCRdocument->createElement('InformationSystem');
		$e_Actor->appendChild($e_InformationSystem);
		$e_Name = $this->CCRdocument->createElement('Name', $informationData['facility']);
		$e_InformationSystem->appendChild($e_Name);
		$e_Type = $this->CCRdocument->createElement('Type', 'Facility');
		...
		$e_Type = $this->CCRdocument->createElement('Type');
		$e_Address->appendChild($e_Type);
		$e_Text = $this->CCRdocument->createElement('Text', 'WP');
		$e_Type->appendChild($e_Text);
		$e_Line1 = $this->CCRdocument->createElement('Line1', $informationData['street']);
		$e_Address->appendChild($e_Line1);
		$e_Line2 = $this->CCRdocument->createElement('Line2');
		$e_Address->appendChild($e_Line1);
		$e_City = $this->CCRdocument->createElement('City', $informationData['city']);
		$e_Address->appendChild($e_City);
		$e_State = $this->CCRdocument->createElement('State', $informationData['state'] . ' ');
		$e_Address->appendChild($e_State);
		$e_PostalCode = $this->CCRdocument->createElement('PostalCode', $informationData['postal_code']);
		$e_Address->appendChild($e_PostalCode);
		$e_Telephone = $this->CCRdocument->createElement('Telephone');
		$e_Actor->appendChild($e_Telephone);
		$e_Phone = $this->CCRdocument->createElement('Value', $informationData['phone']);
		...
		$e_Actor = $this->CCRdocument->createElement('Actor');
		$e_Actors->appendChild($e_Actor);
		$e_ActorObjectID = $this->CCRdocument->createElement('ActorObjectID', $this->gaiaID);
		$e_Actor->appendChild($e_ActorObjectID);
		$e_InformationSystem = $this->CCRdocument->createElement('InformationSystem');
		$e_Actor->appendChild($e_InformationSystem);
		$e_Name = $this->CCRdocument->createElement('Name', 'GEHR');
		$e_InformationSystem->appendChild($e_Name);
		$e_Type = $this->CCRdocument->createElement('Type', 'GaiaEHR');
		$e_InformationSystem->appendChild($e_Type);
		$e_Version = $this->CCRdocument->createElement('Version', '1.x');
		$e_InformationSystem->appendChild($e_Version);
		$e_IDs = $this->CCRdocument->createElement('IDs');
		$e_Actor->appendChild($e_IDs);
		$e_Type = $this->CCRdocument->createElement('Type');
		$e_IDs->appendChild($e_Type);
		$e_Text = $this->CCRdocument->createElement('Text', 'Certification #');
		$e_Type->appendChild($e_Text);
		$e_ID = $this->CCRdocument->createElement('ID', 'NONE');
		$e_IDs->appendChild($e_ID);
		$e_Source = $this->CCRdocument->createElement('Source');
		$e_IDs->appendChild($e_Source);
		$e_SourceActor = $this->CCRdocument->createElement('Actor');
		...
		$e_Text = $this->CCRdocument->createElement('Text', 'WP');
		$e_Type->appendChild($e_Text);
		$e_Line1 = $this->CCRdocument->createElement('Line1', '90 Blvd. Media Luna');
		$e_Address->appendChild($e_Line1);
		$e_Line2 = $this->CCRdocument->createElement('Line2');
		$e_Address->appendChild($e_Line1);
		$e_City = $this->CCRdocument->createElement('City', 'San Juan');
		$e_Address->appendChild($e_City);
		$e_State = $this->CCRdocument->createElement('State', 'PR ');
		$e_Address->appendChild($e_State);
		$e_PostalCode = $this->CCRdocument->createElement('PostalCode', '00987');
		$e_Address->appendChild($e_PostalCode);
		$e_Telephone = $this->CCRdocument->createElement('Telephone');
		$e_Actor->appendChild($e_Telephone);
		$e_Phone = $this->CCRdocument->createElement('Value', '000-000-0000');
		$e_Telephone->appendChild($e_Phone);
		$e_Source = $this->CCRdocument->createElement('Source');
		$e_Actor->appendChild($e_Source);
		$e_Actor = $this->CCRdocument->createElement('Actor');
		...
		foreach($labData AS $row)
        {
			$e_Actor = $this->CCRdocument->createElement('Actor');
			$e_Actors->appendChild($e_Actor);
			$e_ActorObjectID = $this->CCRdocument->createElement('ActorObjectID', ${"labID{$row['id']}"});
			$e_Actor->appendChild($e_ActorObjectID);
			$e_InformationSystem = $this->CCRdocument->createElement('InformationSystem');
			$e_Actor->appendChild($e_InformationSystem);
			$e_Name = $this->CCRdocument->createElement('Name', $row['lname'] . " " . $row['fname']);
			$e_InformationSystem->appendChild($e_Name);
			$e_Type = $this->CCRdocument->createElement('Type', 'Lab Service');
			$e_InformationSystem->appendChild($e_Type);
			$e_IDs = $this->CCRdocument->createElement('IDs');
			$e_Actor->appendChild($e_IDs);
			$e_Type = $this->CCRdocument->createElement('Type');
			...
			$e_Text = $this->CCRdocument->createElement('Text', 'WP');
			$e_Type->appendChild($e_Text);
			$e_Line1 = $this->CCRdocument->createElement('Line1', $row['street']);
			$e_Address->appendChild($e_Line1);
			$e_Line2 = $this->CCRdocument->createElement('Line2');
			$e_Address->appendChild($e_Line1);
			$e_City = $this->CCRdocument->createElement('City', $row['city']);
			$e_Address->appendChild($e_City);
			$e_State = $this->CCRdocument->createElement('State', $row['state'] . ' ');
			$e_Address->appendChild($e_State);
			$e_PostalCode = $this->CCRdocument->createElement('PostalCode', $row['zip']);
			$e_Address->appendChild($e_PostalCode);
			$e_Telephone = $this->CCRdocument->createElement('Telephone');
			$e_Actor->appendChild($e_Telephone);
			$e_Phone = $this->CCRdocument->createElement('Value', $row['phone']);
			$e_Telephone->appendChild($e_Phone);
			$e_Source = $this->CCRdocument->createElement('Source');
			$e_Actor->appendChild($e_Source);
			$e_Actor = $this->CCRdocument->createElement('Actor');
