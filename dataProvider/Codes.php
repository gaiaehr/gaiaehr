...
class Codes
{
	 ...
	 *
	 * Function is able to search a variety of code sets. See the 'external' items in
	 * the comments at top
	 * of this page for a listing of the code sets supported. Also note that Products
	 * (using PROD as code type)
	 * is also supported.
	 *
	 * @param  string    $code_type  code set key (special keywords are PROD and
	 * --ALL--)
	 * @param  string    $search_term     search term
	 * @param  boolean   $count           if true, then will only return the number
	 * of entries
	 * @param  boolean   $active          if true, then will only return active
	 * entries (not pertinent for PROD code sets)
	 * @param  boolean   $only_one if true, then will only return one perfect
	 * matching item
	...
	// $search_term . "%"));
	//		} else if($code_type == '--ALL--') { // Search all codes from the default
	// codes table
	...
	//			}
	//			$query = "SELECT c.id, c.code_text, c.code_text_short, c.code, c.code_type,
	// c.modifier, c.units, c.fee, " .
	//				"c.superbill, c.related_code, c.taxrates, c.cyp_factor, c.active,
	// c.reportable, c.financial_reporting, " .
	...
	//			$res   = sqlStatement($query, array("%" . $search_term . "%", "%" .
	// $search_term . "%"));
	//		} else if(!($code_types[$code_type]['external'])) { // Search from default
	// codes table
	//			$active_query = '';
	//			if($active) {
	//				// Only filter for active codes
	...
	//			$query          = "SELECT c.id, c.code_text, c.code_text_short, c.code,
	// c.code_type, c.modifier, c.units, c.fee, " .
	//				"c.superbill, c.related_code, c.taxrates, c.cyp_factor, c.active,
	// c.reportable, c.financial_reporting, " .
	...
	//		} else if($code_types[$code_type]['external'] == 1) { // Search from ICD10
	// diagnosis codeset tables
	//			$active_query = '';
	//			if($active) {
	//				// Only filter for active codes
	...
	//				$sql_bind_array = array();
	//				$query          = "SELECT ref.formatted_dx_code as code, ref.long_desc as
	// code_text, " .
	...
	//				array_push($sql_bind_array, $code_types[$code_type]['id']);
	//				if($only_one) {
	//					$query .= "WHERE ref.formatted_dx_code = ? AND ref.valid_for_coding = '1'
	// AND ref.active = '1' $active_query $query_filter_elements ";
	//					array_push($sql_bind_array, $search_term);
	//				} else {
	//					$query .= "WHERE (ref.long_desc LIKE ? OR ref.formatted_dx_code LIKE ?)
	// AND ref.valid_for_coding = '1' AND ref.active = '1' $active_query
	// $query_filter_elements ";
	//					array_push($sql_bind_array, "%" . $search_term . "%", "%" . $search_term .
	// "%");
	//				}
	//				$query .= "ORDER BY ref.formatted_dx_code+0, ref.formatted_dx_code
	// $limit_query";
	//				$res = sqlStatement($query, $sql_bind_array);
	//			}
	//		} else if($code_types[$code_type]['external'] == 2) { // Search from SNOMED
	// (RF1) diagnosis codeset tables
	//			if($active) {
	//				// Only filter for active codes
	//				// If there is no entry in codes sql table, then default to active
	//				//  (this is reason for including NULL below)
	...
	//			if(!(empty($check_table))) {
	//				$sql_bind_array = array();
	//				$query          = "SELECT ref.ConceptId as code, ref.FullySpecifiedName as
	// code_text, " .
	//					"c.id, c.code_type, c.modifier, c.units, c.fee, " .
	//					"c.superbill, c.related_code, c.taxrates, c.cyp_factor, c.active,
	// c.reportable, c.financial_reporting, " .
	...
	//				array_push($sql_bind_array, $code_types[$code_type]['id']);
	//				if($only_one) {
	//					$query .= "WHERE (ref.ConceptId = ? AND ref.FullySpecifiedName LIKE
	// '%(disorder)') $active_query $query_filter_elements ";
	//					array_push($sql_bind_array, $search_term);
	//				} else {
	//					$query .= "WHERE ((ref.FullySpecifiedName LIKE ? OR ref.ConceptId LIKE ?)
	// AND ref.FullySpecifiedName LIKE '%(disorder)') $active_query
	// $query_filter_elements ";
	//					array_push($sql_bind_array, "%" . $search_term . "%", "%" . $search_term .
	// "%");
	//				}
	...
	//				$res = sqlStatement($query, $sql_bind_array);
	//			}
	//		} else if($code_types[$code_type]['external'] == 3) { // Search from SNOMED
	// (RF2) diagnosis codeset tables
	//			//placeholder
	//		} else {
	//			if($code_types[$code_type]['external'] == 4) { // Search from ICD9 diagnosis
	// codeset tables
	//				if($active) {
	//					// Only filter for active codes
	//					// If there is no entry in codes sql table, then default to active
	...
	//					array_push($sql_bind_array, $code_types[$code_type]['id']);
	//					if($only_one) {
	//						$query .= "WHERE ref.formatted_dx_code = ? AND ref.active = '1'
	// $active_query $query_filter_elements ";
	//						array_push($sql_bind_array, $search_term);
	//					} else {
	//						$query .= "WHERE (ref.long_desc LIKE ? OR ref.formatted_dx_code LIKE ?)
	// AND ref.active = '1' $active_query $query_filter_elements ";
	...
	//				}
	//			} else {
	//				if($code_types[$code_type]['external'] == 5) { // Search from ICD9
	// Procedure/Service codeset tables
	//					if($active) {
	//						// Only filter for active codes
	//						// If there is no entry in codes sql table, then default to active
	//						//  (this is reason for including NULL below)
	...
	//					if(!(empty($check_table))) {
	//						$sql_bind_array = array();
	//						$query          = "SELECT ref.formatted_sg_code as code, ref.long_desc as
	// code_text, " .
	//							"c.id, c.code_type, c.modifier, c.units, c.fee, " .
	//							"c.superbill, c.related_code, c.taxrates, c.cyp_factor, c.active,
	// c.reportable, " .
	...
	//						array_push($sql_bind_array, $code_types[$code_type]['id']);
	//						if($only_one) {
	//							$query .= "WHERE ref.formatted_sg_code = ? AND ref.active = '1'
	// $active_query ";
	//							array_push($sql_bind_array, $search_term);
	//						} else {
	//							$query .= "WHERE (ref.long_desc LIKE ? OR ref.formatted_sg_code LIKE ?)
	// AND ref.active = '1' $active_query ";
	//							array_push($sql_bind_array, "%" . $search_term . "%", "%" . $search_term
	// . "%");
	//						}
	//						$query .= "ORDER BY ref.formatted_sg_code+0, ref.formatted_sg_code
	// $limit_query";
	//						$res = sqlStatement($query, $sql_bind_array);
	//					}
	//				} else {
	//					if($code_types[$code_type]['external'] == 6) { // Search from ICD10
	// Procedure/Service codeset tables
	//						$active_query = '';
	//						if($active) {
	//							// Only filter for active codes
	...
	//						if(!(empty($check_table))) {
	//							$sql_bind_array = array();
	//							$query          = "SELECT ref.pcs_code as code, ref.long_desc as
	// code_text, " .
	//								"c.id, c.code_type, c.modifier, c.units, c.fee, " .
	//								"c.superbill, c.related_code, c.taxrates, c.cyp_factor, c.active,
	// c.reportable, " .
	...
	//							array_push($sql_bind_array, $code_types[$code_type]['id']);
	//							if($only_one) {
	//								$query .= "WHERE ref.pcs_code = ? AND ref.valid_for_coding = '1' AND
	// ref.active = '1' $active_query ";
	//								array_push($sql_bind_array, $search_term);
	//							} else {
	//								$query .= "WHERE (ref.long_desc LIKE ? OR ref.pcs_code LIKE ?) AND
	// ref.valid_for_coding = '1' AND ref.active = '1' $active_query ";
	//								array_push($sql_bind_array, "%" . $search_term . "%", "%" .
	// $search_term . "%");
	//							}
	...
	//			return array('success' => true, 'error' => array('zippedCodes' =>
	// $this->zippedCodes, 'params' => $params, 'file' => $file));
	//		} else {
